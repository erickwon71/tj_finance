...
> **상태(2026-08-16 갱신 #2)**: **T3(R29) 구현+백필+검증+커밋 완료**(`4fb5eef`, push 대기).
> **T4는 T4-1(원문대조 5건)+T4-2b(분류) 완료 — 3개 메커니즘 확정, M3만 코드로 구현(R30),
> M1/M2는 사용자 결정으로 보류(문서화만).**
> **T1/T2는 여전히 설계·측정만 완료, 구현 미착수·사용자 승인 대기**
> (`CLAUDE.md` 정책, [[feedback-plan-then-wait]]).
> **상태(2026-08-17 갱신 #3)**: **T1 그룹A(T22) 별도 트랙 승격 후 R31로 구현+775개사 표적
> 백필+검증까지 완료**(`docs/plans/t22_hyphen_negative_gate_todo_2026-08-16.md`,
> `docs/PARSING_RULES.md` R31). T1 잔여 **13 → 8**(그룹A 5/6 해소, 1건은 별개 신규결함으로
> 재분류·범위밖. 그룹B 7건은 예정대로 T4/M2 범위 불변). T2는 여전히 미착수.
...
| **T1** | 잔여 13키 무손실 불변식 위반 | 13행 (A 6 / B1 3 / B2 4) | 부분적 | 하 | ✅완료(2026-08-17, 그룹A=T22=R31로 구현. 13→8) | 4순위 |
...
#### 그룹 A — 6키: 현재 코드는 **정상 emit**, `col_index≥1`이라 적재 정책에 걸림 (★원인 확정, 2026-08-16 재조사)
...
**판정 — (b) 열 귀속 결함으로 확정. "손실 아님"(a) 가설은 반증됐다.** 원문 XML을 lxml로 직접
파싱하고 프로덕션 함수(`_split_label_amounts`/`extract_rows`)를 그대로 호출해 재현했다
(20031114000665, 20040619000015 2건 원문대조, `--mode trace`).

**근본 원인 — `parser/xml/table_extractor.py:36-47`의 `_NUMBER_PATTERN`이 "부호 없는 하이픈
음수"(`-466,274` 형태, 괄호도 △/▲도 없는 순수 `-`+숫자)를 인식하는 대안이 없다.**

```python
_NUMBER_PATTERN = re.compile(
    r'^[\s\-\─\—\―]$|'          # 공란/대시 "글자 하나"만 (숫자 아님)
    r'^\([\d,]+\.?\d*\)$|'       # (1,234) 괄호음수
    r'^\(-\)[\d,]+\.?\d*$|'      # (-)1,234 — R19/T21 에서 이미 커버
    r'^[\d,]+\.?\d*$|'           # 1,234 양수 — 선행 "-" 없음
    r'^△[\d,]+\.?\d*$|'          # △1,234
    r'^▲[\d,]+\.?\d*$'           # ▲1,234
)
```

`-466,274`는 위 6개 대안 중 어디에도 안 걸린다(첫 대안은 "-" **한 글자만**인 셀만 잡는다).
`_split_label_amounts`(같은 파일 498줄)는 매칭 안 되는 셀을 **"숫자 아닌 텍스트 → 무시"로
완전히 드롭**한다(빈 문자열 placeholder조차 안 남긴다) — `parse_amount`는 실제로 `-466,274`를
정상적으로 음수 처리하지만(`amount_normalizer.py:344`), 그 앞의 **게이트에서 이미 걸러져
`parse_amount`까지 도달하지 못한다.** T21(`(-)N` 이중표기)과 **정확히 같은 결함 계열**이지만
T21이 고친 건 "`(-)` 이중마커" 하나뿐이고 **순수 `-` 단일마커는 그때도 지금도 미수정**이다.

**20031114000665 원문대조(실측, `_split_label_amounts` 직접 호출로 재현)**:
표 헤더 `제43기 분기[3개월|누적] × 제42기 분기[3개월|누적]`, 데이터 행은 8개 금액 TD
`['', '-466,274', '', '3,616,480', '', '-493,768', '', '4,278,634']`. 음수 2칸(`-466,274`,
`-493,768`)이 게이트에서 드롭되며 나머지 값들이 배열 안에서 위치가 밀린다. 그 결과
`_interim_cumulative_cols`가 헤더 위치 기준으로 만든 `cum_map={1:0, 3:1}`(헤더 관점 "당기누적
컬럼은 인덱스1")이 실제로는 밀린 배열의 **엉뚱한 자리**를 가리키게 돼, 진짜 당기 누적값
(3,616,480)은 아예 조회 안 되고 **전기(42기) 누적값(4,278,634)이 `col_index=1`로 잘못 emit**된다
(그마저도 col≥1이라 적재 정책에서 또 걸러진다).

**20040619000015 원문대조**: 6개 금액 TD `['', '239,103', '', '832,350', '1,159,264',
'-1,919,199']` 중 마지막 셀(16기 연간, `-1,919,199`)이 같은 게이트로 드롭돼 배열이 5칸으로
줄어들고, 앞쪽 None 압축 로직이 또 얹혀 결과적으로 17기 연간값(1,159,264, 원래는 당기/전기
어느 쪽도 아닌 참고용 연간 비교 컬럼)이 `col_index=1`로 emit된다.

**함의 — 이건 13건짜리 문제가 아니다.** `_split_label_amounts`는 BS/IS/CF/SCE/APPR·본문·주석
전체가 공유하는 전역 게이트다. 순수 하이픈 음수 셀이 있는 **모든 표**(2단 헤더 interim
cum_map 표만이 아니라)에서 같은 방식으로 셀이 통째로 사라지고 뒤 컬럼이 밀린다 — 잃어버린 것만
(이 13건처럼) 눈에 띄고, **드러나지 않은 채 다른 행에서 엉뚱한 컬럼값이 조용히 적재됐을 가능성이
있다**(T21이 이미 겪은 것과 같은 패턴: "결측보다 나쁜, 틀린 숫자가 조용히 들어감"). 스코프
측정은 이번 세션에서 하지 않았다 — **별도 트랙 승격 여부는 사용자 결정 필요**.
`docs/PARSING_RULES.md` 부록 A에 **T22**로 신규 등재(미수정, open).
...
#### 그룹 B2 — 4키: T4/M2와 동일 메커니즘으로 확정(★원인 규명 완료, 2026-08-16 — "미규명"·"라벨 흡수" 가설 둘 다 반증됨)

20020330000386 / 20060512002144 / 20070330001121 / 20070814000766 —
`extract_rows`까지는 행이 도달하지만 `raw_amounts`가 `['', '', '']`처럼 **전부 빈 문자열**이다.

기존 가설("라벨이 거대 병합 문자열이라 `_split_label_amounts`가 금액 셀까지 흡수")은 **반증됐다**.
20020330000386 원문대조(lxml 직접 파싱): 실제 데이터 행은 `['XⅢ.당기순이익(주석15)...', '',
'20,754,252,872', '', '16,620,126,972']` — 금액 셀 2개가 라벨과 분리된 채 **온전히 존재**한다.
`_split_label_amounts(cells)`를 이 셀들로 직접 호출하면 `amount_cells=['', '20,754,252,872', '',
'16,620,126,972']`로 **정상 분리**됨을 확인(라벨 흡수는 일어나지 않는다).

**진짜 원인 — T4(§5)의 M2(`doc_default` 오적용)와 완전히 같은 결함이다.** 이 표 직전에
`(단위 : 원)`이 명시적으로 선언돼 있는데(원문 확인) 코드가 이를 못 찾아 `doc_default` 폴백으로
**×1,000,000(백만원) 배수를 잘못 적용**했다. `scripts/probe_unit_overscale_2026-08-16_results.csv`·
`scripts/scan_t4_2b_m2_m3_split_2026-08-16_results.csv`에 **B2 4키 전부가 이미 `unit_source=
doc_default, adecimal=-6, classification=M2`로 잡혀 있었다**(T4 세션에서 발견은 됐지만 T1과
연결짓지 않았던 것). `parse_amount("20,754,252,872", 1000000)` = `None`(R3 상한 1×10¹⁶ 초과,
실측 재현) — `parse_amount("...", 1)`(올바른 배수)로는 정상 파싱된다.

`raw_amounts`가 완전히 빈 문자열인 이유: `extract_rows`의 "6-column 대응 선행 None 압축" 루프가
`all_parsed`(전부 `None` — 위 R3 상한으로 양쪽 값 다 파싱 실패)를 **끝까지 pop**해 `all_raw`도
함께 통째로 사라진다. 즉 "셀이 원래 공란"이 아니라 **T4/M2가 값을 죽인 뒤, 압축 로직이 그
흔적(원문 텍스트)까지 지운 것**이다.

→ **B2 4키는 T4의 대상이다.** T4는 이번 세션에 M3(구조적 코드버그, 30그룹 중 23그룹)만
구현하고 M2(98.7%, 이 4키 포함)는 "재론 시 문서 §5-6/§5-7-1/§5-8부터"로 **사용자 결정에 따라
보류**된 상태다([[eps-r28-followup-tracks-design-2026-08-16]] 메모리 참고) — 새 작업 아님,
분류만 정리됐다.
...
### 5-6. T4-1 원문대조 5건 — 결과 (2026-08-16, ✅완료 · 5/5 경로 특정)

**표본**: `doc_default` 3건(legacy 2건 + 2020+ 1건) + `declared` 2건(legacy 1건 + 2020+ 1건),
CF/SCE/BS 3종에 걸쳐 선정(§5-5 기준의 "declared 2 + doc_default 2 + 2020+ 1"을 만족하고
BS까지 넓혔다). 재현: `.venv/bin/python scripts/verify_t4_1_source_2026-08-16.py`.

| # | rcept_no | corp | 기간 | statement | unit_source | 판정 |
|---|---|---|---|---|---|---|
| 1 | 20040623000255 | 00117212 (두산) | 2003FY | CF/consolidated | doc_default | **M2** — 표제표 단위셀이 `(단위 : )` **빈 값** |
| 2 | 20050516001328 | 00153393 (태광산업) | 2005Q1 | SCE/consolidated | doc_default | **M2** — 표제표에 단위 선언 **자체가 없음** |
| 3 | 20130828000220 | 00359623 (우리산업홀딩스) | 2013H1 | SCE/consolidated | declared | **M1** — 표제표가 `(단위 : 백만원)`을 **명시 선언**, 그러나 원문 셀 값 자체가 이미 원 단위 |
| 4 | 20220322000790 | 00176835 (NH농우바이오) | 2021FY | SCE/consolidated | declared | **M1** — #3과 동일 패턴(2020년 이후에도 재현) |
| 5 | 20220317001218 | 01113499 (아이티센피엔에스) | 2021FY | BS/separate | doc_default | **M3** — 표제표가 `(단위: 원)`을 정확히 선언하지만, 그 사이에 안내문 `<P>`가 끼어 있어 코드가 못 읽음 |

**공통 검증(5/5)**: DB의 `value_won` = 원문 raw × 적용배수(선언 또는 doc_default)와
정확히 일치한다 — 즉 **곱셈 자체는 코드가 말하는 그대로 정직하게 수행됐다**. 버그는
"곱하기"가 아니라 "무엇을 곱할지 정하는 단계"에 있다.

**세 가지 서로 다른 메커니즘으로 갈린다** (당초 설계의 "①선언 오독 ②상속 오적용
③원문 자체 이상" 3분류와 대응하되, 실측으로 이름을 더 구체화했다):

#### M1 — "declared", 표제표는 단위를 명시했지만 그 선언이 실제 셀 값과 안 맞는다 (원문 자체 결함)

표본 #3(우리산업홀딩스): 표제표 `<TD>(단위 : 백만원)</TD>`가 SCE 데이터표 바로 앞에
있다 — 코드가 "선언을 못 찾은" 게 아니라 **선언을 정확히 읽어서 정직하게 적용**했다.
문제는 선언 자체가 그 표의 실제 값과 안 맞는다는 것 — `2011.01.01(기초자본)` 행의
자본금 셀이 `6,077,715,000`인데, 이게 백만원 단위면 자본금이 6.08경원(우리산업홀딩스
전체 시가총액을 수만 배 초과)이 되어 물리적으로 불가능하다. 실제로는 원 단위 값이다.

표본 #4(NH농우바이오, 2021FY)에서 **동일 패턴이 2020년 이후에도 재현**됐다 —
K-GAAP 레거시(pre-2015)만의 문제가 아니라 지금도 진행 중인 관행/결함이다.

추가로 표본 5개와 별도로 코오롱글로벌(00152880, 20210813001272, 2021H1 CF
consolidated, declared) 1건을 더 확인했다 — SCE뿐 아니라 **CF에서도 동일 패턴**
(표제표 `(단위 : 백만원)` 선언, 실제 셀은 원 단위: `영업활동현금흐름 34,993,490,628`)이
재현됐다. §5-7의 SQL 분해가 보여주듯 `declared` 버킷은 BS/CF/IS/SCE/APPR **5개
재무제표 전부**에 걸쳐 있고 SCE(30.7%)가 최다지만 단일 지배 패턴은 아니다.

**계층2 원칙과 정면으로 부딪힌다**: 선언을 있는 그대로 읽었으므로 **파싱 결함이
아니다** — 원문(또는 원문을 만든 DART 서식/제출인)이 스스로 모순된 정보를 실었다.
[[layer2-unit-column-attribution]] 원칙(원문 충실전사, 값 크기로 단위 추론 금지)을
곧이곧대로 적용하면 **여기는 고칠 게 없다** — DB는 원문이 선언한 대로 정직하게
담았을 뿐이다. 그런데 그 결과가 물리적으로 불가능한 값이라 하류(Gate B, 스크리너,
전 화면)에 오염이 그대로 흘러간다. **이건 "파서 버그 수정"이 아니라 "물리적으로
불가능한 값을 만난 정책 결정"** — quarantine(별도 status로 격리) 후보다.

#### M2 — "doc_default", 이 표엔 로컬 선언이 정말로 없다 (해당 표에 한해 근거 자체가 없음)

표본 #1(두산): 표제표의 단위 셀이 `(단위 : )` — **문법적으로는 있지만 내용이 빈 값**.
표본 #2(태광산업): 표제표에 단위를 적는 줄 자체가 **존재하지 않는다**.
두 경우 다 `declared_unit()`이 반환할 게 없어 `doc_default_unit`(문서 다른 곳,
보통 BS/IS 요약표의 선언)으로 최후 폴백한다 — 그런데 그 표(SCE/CF)의 실제 단위가
그 문서 전체 기본값과 다르다. 두산 사례에서 실제 확인: 같은 문서의 별도 BS는
`백만원`이 맞지만, 연결 CF는 **원** 단위로 작성돼 있다(부속 대손충당금 명세 소표는
독립적으로 백만원이 맞는 작은 값이라 오염 없음 — 표마다 실제 단위가 다르다).

**doc_default 자체의 설계 가정("문서 하나 = 단위 하나")이 SCE/CF 성격상 깨지는
경우가 있다**는 뜻 — R4-1([[doc-default-unit-r4-1]])이 도입될 때 검증한 표본에
이런 SCE/CF 불일치 사례가 없었을 뿐, 이 표본이 늘어나며 처음 드러난 한계다.

#### M3 — "doc_default"지만 진짜 선언은 구조적으로 살아 있다 (코드가 못 읽은 경우, 유일한 순수 코드 버그)

표본 #5(아이티센피엔에스): 표제표가 `(단위: 원)`을 정확히 선언한다. 그런데 그 표제표와
데이터표 사이에 안내 문단(`<P>※상기 제 11기 재무상태표에 대한 사항은 주주총회 승인
전 재무제표이며 …</P>`)이 하나 끼어 있다.

`fin2/extract/text.py:458-496 declaration_text()`의 (3)번 절(요약재무정보 클러스터
소급)은 `_is_metadata_only()`가 아닌 형제를 만나면 즉시 멈춘다(`text.py:493-494`) —
이 안내문은 완전한 문장이라 메타데이터가 아니므로 여기서 멈춘다.
`fin2/extract/text.py:533-584 inherited_declaration_text()`도 태그가 TABLE이 아닌
비어있지 않은 형제를 만나면 **그 형제 하나만 보고** 판정을 끝낸다(`text.py:570-582`,
`continue` 없이 종료) — 더 뒤로 가지 않는다.

즉 두 함수 다 "표제표 → (빈 요소만) → 데이터표"라는 인접성을 전제하는데, 이 문서는
"표제표 → 안내문단 → 데이터표"라서 **둘 다 표제표에 도달하기도 전에 멈춘다**.
정보는 원문에 구조적으로 존재하고 위치도 명확한데, 현재 룩백 로직의 "완전한 문장을
만나면 멈춘다"는 정지 조건이 **이 특정 안내문 패턴을 걸러내지 못한다**.

→ **이게 유일하게 "코드를 고치면 값이 정확해지는" 케이스다.** 단, LVMC 회귀
(text.py:449-450, 남의 표 단위를 잘못 주워온 사고)가 보여주듯 룩백 범위를 넓히는
수정은 **항상 새 회귀 위험을 동반**하므로, "안내문/보류문구"만 안전하게 통과시키는
좁은 조건이 필요하다(§5-8에서 다룬다).

### 5-7. T4-2b 자동 분류 — 결과 (2026-08-16, DB 사실 기반 · doc_default 내부 M2/M3 분해는 표본 추정)

**M1 vs {M2,M3} 경계는 `unit_source` 컬럼과 정확히 일치한다** — `declared`는 항상 M1
(코드가 선언을 찾아 정직하게 적용), `doc_default`는 항상 M2 또는 M3(코드가 로컬 선언을
못 찾아 폴백)이다. 이건 근사가 아니라 **코드 경로 자체의 이분법**이므로 SQL로 정확히
센다(§5-2 A와 동일 쿼리, 여기서는 5-7-1로 통계별 분해):

```
M1 (declared) 후보          : 11,150행 / 364필링 / 172개사
{M2,M3} (doc_default) 후보  : 11,570행 / 559필링 / 256개사
```

**M1 후보의 재무제표 분포** (declared, `>10¹⁵`) — SCE가 최다지만 5종 전부에 있다:

```
SCE  3,420행/113필링   BS  3,097행/167필링   CF  2,881행/147필링
IS   1,708행/240필링   APPR   44행/  8필링
```

**M1 후보의 연도 분포** — pre-2015 구K-GAAP만의 문제가 **아니다**, 오히려 절반
이상이 2020년 이후다(§5-6 코오롱글로벌·NH농우바이오로 실측 재확인):

```
2000~ 596  2005~ 1,470  2010~ 1,110  2015~ 1,942  2020~ 5,831  2025~ 201
```

**{M2,M3} 후보의 재무제표·연도 분포** — 반대로 pre-2015에 96.5% 집중돼 있다
(CF 4,399·BS 2,905·IS 2,646·APPR 1,136·SCE 484 / era 2000~2005 합계 11,168행,
2015년 이후는 398행뿐). §5-2 관찰 1("2020년 이후 6,047행도 있다")은 **M1(declared)
쪽**이 절대다수였다는 게 이번 분해로 분명해졌다 — {M2,M3}는 사실상 레거시 문제다.

### 5-7-1. {M2,M3} 내부 분해 — 전수 스캔 완료 (2026-08-16, 사용자 실행)

3건 표본(M2:M3=2:1) 추정으로는 방향을 정하기엔 근거가 약해, `doc_default` 667개
(rcept_no, statement, basis) 그룹 **전체**를 재파싱해 분류했다
(`scripts/scan_t4_2b_m2_m3_split_2026-08-16.py`, 장시간 명령이라 사용자가 실행 —
[[feedback-long-running-commands]]). 방법: ①`extract_report_lines()`와 **동일한 라우팅
함수**(`_detect_pre2015_body_statement_tables_merged`/`_detect_body_statement_tables`)로
해당 statement/basis의 후보 표만 좁혀 원문 raw 값(콤마 포함 문자열)이 어느 표에 있는지
찾고 ②그 표에서 `declaration_text()/inherited_declaration_text()`(운영 코드 그대로)가
정말 None을 반환하는지 sanity-check한 뒤 ③**안내문단(텍스트 있는 형제)만 건너뛰도록
확장한 룩백**(`_extended_probe`, `text.py`의 TABLE 처리 로직은 그대로 재사용 — 상속
가능한 "선언전용 표"는 정상 인정하고, 빈/무선언 표는 그대로 정지시켜 M2 신호를 안
지운다)으로 그 확장이 선언을 찾아내는지 본다.

```
667개 그룹(전수) 결과:
  M2                              614그룹 / 10,914행  (94.4% of doc_default 11,570행)
  M3                                30그룹 /    288행  ( 2.5%)
  sanity_fail(모호한 표 매칭)       18그룹 /    262행  ( 2.3%, 측정잡음 — 아래 설명)
  file_missing                       4그룹 /    102행
  table_not_found                    1그룹 /      4행
```

**결론이 표본 추정(2:1)보다 훨씬 더 M2 쪽으로 쏠렸다** — 행 기준 M2:M3 = 약 38:1.
`sanity_fail`(운영 코드가 실제로 선언을 찾아버린 경우, 18건)은 대부분 같은 값이
BS와 SCE에 동시에 나타나는(자본총계 등) 경우 스캔의 값-매칭이 두 후보 표 중
**엉뚱한 쪽**을 집었기 때문으로 확인됨(예: 01032486 두산밥캣 2024, BS 질의인데
SCE의 선언표를 집음) — DB의 `unit_source='doc_default'` 분류 자체가 틀렸다는
근거는 아니다(정상 sanity 통과율 97.3%가 이를 뒷받침).

**M3(30그룹)는 13개사에 몰려 있다** — 기업은행(00149646) 9건·00122898 5건·
00163345 3건이 57%를 차지. **기업은행 표본(20050331001780, 대차대조표)을 직접 열어
확인**: 표제표("대차대조표...(단위 : 백만원)")와 데이터표 사이에 낀 게 §5-6의
"긴 안내문"이 아니라 `<P>(은행계정)</P>` — **은행업 특유의 짧은 괄호 계정구분
라벨**("은행계정" vs "신탁계정", [[std-v3-side-findings-trust-account-net-income-2026-08-09]]과
같은 계열 구조)이었다. 즉 M3는 **하나의 안내문 패턴이 아니라 최소 2종의 서로
다른 "차단 요소"**(① 장문 공시 유보문구 — 아이티센피엔에스 ② 은행/보험업 계정구분
짧은 괄호라벨 — 기업은행류)가 있고, 둘 다 특정 회사·업종에 집중된다(일반화된
전사적 패턴이 아니다).

**T4-3에 주는 의미**: M3(코드로 안전하게 고칠 여지가 있는 부분)는 최대치로 잡아도
**22,720행 중 288행(1.3%)뿐**이다. 나머지 98.7%(M1 11,150행 + M2 10,914행 + 소량
미해결)는 "선언을 못 읽어서"가 아니라 "선언이 없거나 선언 자체가 모순"인 경우라
**코드 수정으로 도달할 수 없다** — quarantine 같은 정책 결정이 문제의 사실상
전부를 좌우한다.

### 5-8. M3 자체는 좁게 고칠 수 있다 (참고용 스케치, 아직 승인 대상 아님)

§5-7-1 전수 스캔으로 M3가 22,720행 중 **288행(1.3%)뿐**임이 확정됐다 — 승인되더라도
효과가 작다는 걸 먼저 인지해야 한다. 그래도 유일한 순수 코드 버그라 방향은 대략
이렇다(★설계 확정이 아니라 다음 세션 출발점 메모):

- 차단 요소가 **최소 2종**으로 갈렸다(§5-7-1): ① 장문 공시 유보문구(아이티센피엔에스류,
  "상기 … 승인 전 재무제표이며 …") ② 은행/보험업 짧은 괄호 계정구분 라벨(기업은행류,
  `(은행계정)`/`(신탁계정)`). 하나의 정규식으로 묶으려 하지 말고 **패턴별로 화이트리스트
  후보를 따로 뽑아야** 오탐이 적다.
- `declaration_text()`의 (3)번 절 정지 조건 `if not _is_metadata_only(t): break`에
  이 화이트리스트를 추가해, 매칭되는 문단은 "완전한 문장이지만 통과"시킨다.
- 화이트리스트를 **좁게** 잡아야 LVMC 회귀(§5-6 M3 설명의 `text.py:449-450`)를
  재현하지 않는다 — 일반적인 "완전한 문장이면 통과"로 풀면 안 된다.
- M3 30그룹이 13개사(57%가 3개사)에 몰려 있다는 것도 감안 대상 — **전사적 개선이
  아니라 소수 회사·업종(은행/보험 계정구분 서식 포함)의 반복 필링을 회수하는
  국지적 수정**이라는 걸 승인 시점에 분명히 해야 한다(288행/23개 필링 규모 대비
  화이트리스트 설계+회귀검증 비용이 맞는지는 사용자 판단 영역).

### 5-9. T4-4 구현 + 백필 + 검증 — 결과 (2026-08-16, ✅완료 · M3만 진행)

사용자 결정(§6 T4-3): **M3만 진행, M1/M2는 문서화만 하고 보류.**

**구현**: `fin2/extract/statement_titles.py::_is_bare_structural_marker(txt)` 신설
— §5-8이 갈랐던 2종(표제 반복 캡션·계정구분 괄호라벨)을 하나의 판정 함수로 통합
(정규식 2개, 닫힌 계정구분 목록 1개). `declaration_text()`(range 3→6)·
`inherited_declaration_text()` 양쪽의 텍스트-형제 분기 최우선에 배선 — TABLE 형제
(LVMC 사고 발생 경로)는 손대지 않았다. 상세는 `docs/PARSING_RULES.md` R30.

**단위테스트 9개** — `fin2/tests/test_declaration_lookback_bare_marker_r30.py`.
합성 XML은 전부 원문 실측 구조 그대로 재현(기업은행 은행계정·서원류 APPR 표제
반복·연쇄 2단 스킵). ★안전판 회귀 테스트 2개 포함: 다른 정보가 섞인 표제는 여전히
경계 유지, 빈 단위선언(`(단위 : )`)은 스킵해도 여전히 채우지 않음(M2를 M3로
오염시키지 않는다).

**사전 검증 — 실제 프로덕션 함수로 30그룹 재검사**(`scripts/verify_m3_fix_2026-08-16.py`
패턴, 스크래치패드): DB에 아직 반영 전 XML 재파싱만으로 **23/30그룹(76.7%)이
`declaration_text()/inherited_declaration_text()`로 복구**됨을 먼저 확인한 뒤
백필을 진행했다. 미복구 7건은 화이트리스트 밖(위아 회사명 단독 마커 1건·인용부호
안내문 1건·SCE 표제표 자체 단위선언이 빈 경우 3건·garbled 문서 값매칭 오탐 1건 —
T4-1 §5-6 서원 SCE 사례가 실은 표제표 단위 자체가 `(단위 : )` 빈 값이라 §5-7-1의
1차 스캔(자체 `_extended_probe`)이 잘못 M3로 분류했던 것으로 재확인, M2 정체
확정)로 **의도적으로 남겼다**.

**백필**: `reload_report_lines_corp.py --corp <13개사>`(M3 30그룹이 걸친 회사,
1,509 filing 재추출, 8분28초) → `build_std_v3.py --corp <13개사> --year-min 1999`.
T3(계층3만 고침)와 달리 **재추출이 필요**했다 — `declaration_text` 결과가 계층2의
`unit` 자체를 바꾸기 때문이다.
...
| 항목 | 결과 | 판정 |
|---|---|---|
| 계층2 스코프 밖 불변 | 전체 report_lines 60,557,582→60,557,818(**+236, 13개사 증가분과 정확히 일치**) — 다른 회사 영향 0 | ✅ |
| std_v3 행 집합 | 13개사 2,352→2,352(신규 0·소실 0, 값만 교정) | ✅ |
| 의도한 효과(행 단위) | 30그룹 재검사(DB) — **23그룹 fixed(`unit_source: doc_default→declared`, `\|value_won\|>10¹⁵` 0건)** / 7그룹 still-bad(예측과 정확 일치). 288행 중 **144행(50.0%) 실복구** | ✅ |
| Gate B | 13개사 fy≥1999 재감사 — pass 1,383 / **fail_a 0** / fail_b 92 / pending 877. fail_b 92건 전부 `cogs`(00108940, 2009~2024)·`revenue`(00149646, 2023~2024) — R30이 건드린 시대(pre-2010 레거시 BS/CF/APPR/SCE)·필드와 전부 무관, 회귀 아님으로 판단 | ✅ |
| pytest | `pytest tests/ fin2/tests/` **542 passed**(533+9), 무관 기존 실패 1건 그대로 | ✅ |

재현: `.venv/bin/python scripts/reload_report_lines_corp.py --corp 00106623,00108940,00111689,00122898,00131197,00146083,00149646,00159254,00163345,00164779,00192499,00292434,00356370` ·
`.venv/bin/python scripts/build_std_v3.py --corp <위 13개사> --year-min 1999` ·
`.venv/bin/python scripts/gateb_audit.py --source v3 --corp-file <13개사 파일> --fy-min 1999 --recheck`
...
☑ T3-7  R29 등재 · 커밋                    (완료 — 커밋 `4fb5eef`, push는 사용자 요청 시 별도 진행)
```
...
☑ T4-1  원문대조 5건                       (완료, §5-6 — 3개 메커니즘 M1/M2/M3 확정)
☑ T4-2b 22,720행 자동 분류                 (완료, §5-7·§5-7-1 — 667그룹 전수 스캔)
☑ T4-3  설계 확정 · 승인                   (완료, 사용자 결정 2026-08-16 — **M3만 진행,
                                             M1/M2는 문서화만 하고 이번 트랙에서는 보류**)
☑ T4-4  구현 + 백필 + 검증                 (완료, §5-9 — 30그룹 중 23그룹/288행 중 144행
                                             복구, Gate B fail_a 0, pytest 542)
☑ T4-5  R30 등재                          (완료, `docs/PARSING_RULES.md` R30. T1 그룹B1은
                                             T4의 M1/M2 범위라 이번엔 해소되지 않음 — T1 문서만 정정)
```

**T4-3 사용자 결정(2026-08-16)**: "M3 코드 버그까지만 수정하고 나머지는 문서에만
남기고 넘어가자." → M1(11,150행, declared인데 원문 선언 자체가 실값과 모순)·
M2(10,914행, 로컬 선언 진짜 없음)의 quarantine 등 정책 결정은 **이번 트랙에서 다루지
않는다** — §5-6/§5-7-1/§5-8에 이미 문서화된 것으로 충분하다고 판단, 재론은 다음
필요 시점으로 미룸.

**T4-1 — 원문대조 5건 (원인 갈래 확정) ✅완료(§5-6)**
- 표본: `doc_default` 3건(legacy 2·2020+ 1) + `declared` 2건(legacy 1·2020+ 1),
  CF/SCE/BS 3종. 재현: `.venv/bin/python scripts/verify_t4_1_source_2026-08-16.py`.
- **결과**: 5/5 전부 "선언 원문 → 적용 배수" 경로가 파일:줄로 특정됐다(§5-6).
  당초 가설 ①②③이 실측으로 **M1(declared·원문모순)/M2(doc_default·로컬선언부재)/
  M3(doc_default·룩백범위밖 코드버그)** 3갈래로 구체화됐다 — ③에 해당하는 게
  M1인데, 예상과 달리 **레거시 전용이 아니고 2020년 이후에도 재현**됐다(§5-6).

**T4-2b — 자동 분류 ✅완료(§5-7, §5-7-1)**
- M1 vs {M2,M3}는 `unit_source` 컬럼(declared/doc_default)과 **정확히 일치**하는
  코드 경로 이분법이라 SQL로 정확히 셌다 — 짐작이 아니다.
- {M2,M3} 내부(순수 코드버그 M3 vs 정책결정 필요 M2) 세부 분해도 **전수 스캔으로
  완료**됐다(§5-7-1, 667개 그룹 전체 재파싱, 사용자 실행) — M2:M3 = **행 기준
  10,914:288(약 38:1)**. 애초 3건 표본(2:1)보다 훨씬 M2 쪽으로 쏠렸다.
- **완료조건 재확인**: M1(declared)이 다수(11,150/22,720=49.1%)이고, §5-8이 논하듯
  이건 "계층2 원칙상 고칠 게 없는" 원문 자체 결함 쪽으로 기운다 — 당초 우려했던
  "③이 다수면 설계 방향이 완전히 달라진다"가 **실제로 일어났다**. 게다가 코드로
  안전하게 고칠 수 있는 M3조차 **22,720행 중 288행(1.3%)뿐**이고 13개사에 몰려
  있다(§5-7-1) — T4는 사실상 "파서를 고치는 트랙"이 아니라 **"물리적으로 불가능한
  값을 만났을 때의 정책을 정하는 트랙"**에 가깝다는 게 이번 세션 결론이다.

**T4-3 — 설계 확정 ⛔사용자 승인**

이번 세션 실측으로 T4는 **단일 결정이 아니라 최소 두 개의 독립된 결정**으로 갈린다:

1. **M1(declared, 11,150행/49.1%) 처리 방향** — 원문이 스스로 모순된 값을 선언한
   경우다(§5-6 M1). "값이 크니 배수를 줄인다"류 수정은 명백히 금지
   ([[layer2-unit-column-attribution]]). 후보는 대략:
   - (a) 손대지 않는다 — DB는 원문을 정직하게 담았을 뿐이라는 원칙을 끝까지 지킨다.
     대신 하류(스크리너·Gate B)가 이 값을 걸러내도록 **quarantine 플래그**만 추가.
   - (b) `_AMOUNT_SANE_MAX`보다 낮은 물리적 상한(예: 1,000조)을 넘는 행을
     `value_won=NULL` + `unit_source='implausible'` 같은 표식으로 **격리**(적재는
     하되 하류 집계에서 자동 제외).
   - (c) 그 외 — 사용자가 다른 방향을 원하면 별도 논의.
2. **M2/M3(doc_default, 11,570행/50.9%) 처리 방향**:
   - M3(코드가 못 읽은 실재 선언, §5-6·§5-8)만 **좁은 화이트리스트로 룩백 확장**해
     읽어낸다 — 이건 순수 코드 버그 수정이라 상대적으로 안전하지만, 정확한 규모는
     아직 표본(1/3)뿐이라 **먼저 M3 후보 필링 전수 스캔**이 선행돼야 한다.
   - M2(로컬 선언 자체가 없는 표, §5-6)는 M1과 같은 성격의 딜레마다 — doc_default가
     이 표엔 안 맞는다는 게 확인됐지만, 대안이 "값을 버린다" 아니면 "다른 문서 내
     신호로 추측한다"(위험) 둘뿐이라 위 M1 (a)/(b)와 같은 정책으로 묶일 가능성이 높다.
- `_AMOUNT_SANE_MAX`(현재 1경) 조정 여부는 **별건으로 다룬다** — 상한을 낮추면
  오염이 결측으로 바뀔 뿐 근본 해결이 아니다.
- 백필 범위를 여기서 확정한다. **R28 286개사로 잡으면 안 된다**(§5-2 관찰 1,
  §5-7이 재확인 — M1은 오히려 최근 필링이 다수다).
...
### 【3순위】 T2 — 헤더규칙 "기수" 전사 소급 백필 (코드는 이미 수정됨) — ✅완료(2026-08-16)

```
☑ T2-1  범위 확정 (DB 측정으로 대체 — SD카드 원문 전수 스캔 불필요했음, 아래 참고)
☑ T2-2  재추출 범위 확정 · 승인(사용자, 49개사)
☑ T2-3  표적 재추출 (note_lines 배선 = 기존 함수 재사용으로 확정)
☑ T2-4  build_std_v3 재빌드
☑ T2-5  검증
☑ T2-6  데일리 배선 무변경 확인
```

**실행 경위 — 계획 대비 경로 단순화**: T2-1(SD카드 원문 전수 스캔)은 실행하지 않았다.
`note_lines WHERE header_hint='기수' AND label_raw ~ '원|%'` **DB 쿼리 자체가 이미 정확한
대상 목록**(49개사·501행·250필링, note_lines가 2015+만 존재해 원문 스캔과 결과가 같음)이라
원문 스캔은 불필요했다. T2-3의 배선 결정도 원설계가 검토한 (i) `--include-notes` 플래그
신설 대신, **코드 조사 중 발견한 기존 함수 `collector/note_lines_sync.py::sync_layer2_lines
(recheck=True)`를 재사용**했다(사용자 승인, 2026-08-16) — docstring에 이미
"파서 개선 소급 반영용"이라 명시된 바로 그 용도였고, 데일리 파이프라인이 쓰는 것과
**동일 함수**라 T2-6(데일리 배선 무변경) 확인이 설계상 자동으로 충족된다(새 call site
자체를 안 만들었으므로).

**결과** (`scripts/backfill_t2_note_header_hint_2026-08-16.py`,
`scripts/t2_note_header_hint_backfill_2026-08-16.json`):
- 49개사, `year_min=2015`, `recheck=True` — 2,212필링·1,880초, 에러 0.
- `header_hint='기수' AND label_raw~'원|%'`: **501 → 3행(99.4% 해소)**, 49개사 중 48개사 완전
  해소. 잔여 3행(00274933, `20160516003569`)은 배당주석이 "보통주:제21기 500원(10%)
  제20기 750원(15%) 제19기 750원(15%)"처럼 **여러 期를 한 셀에 나열한 복합 라벨**이라
  R28이 고친 단일 패턴과 다른 별개 엣지케이스로 확인됨(오늘 범위 밖, 문서화만).
- **원문대조 1건**(강남제비스코 00100939, `20160330001906`, "1. 배당금 [...제64기: 500원
  (50%) 제63기: 500원(50%)]"): DB `col_index=0/2 = 3,250,000,000원`이 원문 XML의 해당
  두 셀과 정확히 일치 — 이전엔 "기수" 오분류로 손실됐던 행이 정상 복구됨을 확인.
- `build_std_v3.py --corp <49개사> --year-min 2015`: 49corp·4,144행, 132초.
- Gate B(`--source v3 --corp-file ... --fy-min 2015 --recheck`): in-scope 3,682행 중
  일치율 99.9%. fail_a 1건·fail_b 4건 전부 원인 확인함(T2와 무관, 아래 참고).
- pytest: 이번 세션 코드 변경 없음(기존 함수 재사용) — 재실행 불필요로 판단, 생략.

**부수발견 — Gate B 신규 fail_a 1건, T2와 무관한 별개 결함(★새 발견, 미착수)**:
00874803(AP위성) 2025 Q3 separate `tax_expense`. `db_won=757,801,768` ≠
`report_won=416,084,201`. 원문 XBRL 대조: `CFY2025dTQQ`(당기) 컨텍스트 값이
416,084,201로 report_won과 일치, `PFY2024dTQQ`(전기 동기 비교컬럼) 값이 757,801,768로
db_won과 일치 — **std_v3(계층3 combine.py)가 당기 컬럼이 아니라 전기 비교컬럼 값을
잘못 채택**한 것으로 원인 확정. T2(계층2 텍스트 추출)와는 계층·메커니즘이 다른 별개
버그이고, 2025 Q3라는 최신 필링이 오늘 처음 감사망에 걸린 것(T3의 "FY2024~2026 신규
수집분은 별도 소관" 패턴과 동일) — 이번 세션 범위 밖으로 판단, 재론 시 XBRL
`ACONTEXT`(dTQQ/dTQA) 선택 로직(`fin2/layer3/combine.py`)부터 볼 것.
fail_b 4건(00142883 revenue×2·00632304 controlling_equity×2)은 전부 REVIEW(비차단)
등급이고 기존에 반복 확인된 "Track B 리더 자체 한계" 부류(00632304는 2022년 정정필링을
소스로 쓰는 비교컬럼 상황)라 깊이 조사하지 않음.
...
### 【4순위】 T1 — 잔여 13키 (목표는 수정이 아니라 규명·문서정정) — ✅완료(2026-08-17, 그룹A=T22=R31 구현)

```
☑ T1-0  13키 복원 + 원인추적 스크립트 영구화
☑ T1-1  그룹A 6건 원문대조 — (b) 열 귀속 결함으로 확정, T22 신규 등재
☑ T1-2  그룹B1 3건 — T4(M2) 후에도 미해소 확인(13→13, T4는 M3만 구현했으므로 예상대로)
☑ T1-3  그룹B2 4건 원인 특정 — T4/M2와 동일 메커니즘으로 확정("미규명"·"라벨 흡수" 가설 반증)
☑ T1-4  본 설계문서 §2 정정 완료(아래 요약 참고)
```

**최종 판정 (13키 전부 원인 규명 완료)**:

| 그룹 | 건수 | 원인 | 상태 |
|---|---|---|---|
| A | 6 | `_NUMBER_PATTERN`(table_extractor.py) 이 순수 하이픈 음수(`-N`)를 인식 못 해 셀이 통째로 드롭 → cum_map 위치 밀림 → 전기값이 col1로 오emit | **✅R31로 수정 완료(2026-08-17)**. 5/6 col_index=0 복구. 잔여 1건(20040619000015)은 별개 신규결함(num_cols/cum_map 폭 산정, T22 범위 밖) 때문에 미해결 — `t22_hyphen_negative_gate_todo_2026-08-16.md` "범위 밖" 참고 |
| B1 | 3 | T4/M2(doc_default 단위 오적용 → R3 상한 초과 → None) | T4 M2 스코프(사용자 결정으로 이번엔 보류) |
| B2 | 4 | B1과 동일(T4/M2) — 기존 "라벨 흡수" 가설은 반증됨 | T4 M2 스코프(위와 동일) |

**T1-2 재확인 결과(실측)**: T4-4 백필(M3만 구현) 후 `probe_eps_r28_residual13_2026-08-16.py`를
재실행해도 잔여는 여전히 **13건**(10건으로 안 줄었다). 원인은 B1의 3건이 T4의 M3(구조적
코드버그)이 아니라 **M2(로컬 선언 자체가 없음, 사용자 결정으로 이번 T4 구현 범위 밖)**였기
때문 — §5-6/§5-9의 M2/M3 분해(`scan_t4_2b_m2_m3_split_2026-08-16_results.csv`)로 재확인.
"T4를 고치면 B1이 자동 해소된다"던 §2-2의 원래 기대는 **정정**: T4 M3 구현으로는 해소 안 됨.

**남은 결정 사항 — T22(그룹 A 신규 버그)를 별도 트랙으로 승격할지**: 스코프(전사 영향 규모)는
아직 측정 안 됨. `_split_label_amounts`가 BS/IS/CF/SCE/APPR·본문·주석 전체의 공용 게이트라
13건보다 훨씬 넓을 가능성이 있다(§2-2 그룹A 상세 참고). 사용자 결정 필요.
...
| `scripts/verify_t4_1_source_2026-08-16.py` | T4-1 | §5-6 원문대조 5건 재현(파일 위치·단위선언 텍스트·DB 대조) |
| `scripts/scan_t4_2b_m2_m3_split_2026-08-16.py` | T4-2b | §5-7-1 M2/M3 전수 분류(667그룹 재파싱, 운영 라우팅+선언탐지 함수 재사용) |
| `scripts/scan_t4_2b_m2_m3_split_2026-08-16_results.csv` | T4-2b | 위 산출물(667그룹 상세, 사용자 실행 결과) |
| `scripts/probe_m3_blockers_2026-08-16.py` | T4-3 설계근거 | 30그룹 각각의 실제 차단요소(skipped 텍스트) 추출 — R30 화이트리스트의 증거 |
| `scripts/verify_m3_fix_2026-08-16.py` | T4-4 사전검증 | 실제 프로덕션 `declaration_text`/`inherited_declaration_text`로 30그룹 재검사(DB 반영 전) |
| `scripts/r30_target_corps_2026-08-16.txt` | T4-4 백필 | M3 30그룹이 걸친 13개사 목록(reload/build_std_v3/gateb_audit 공용) |
| `fin2/tests/test_declaration_lookback_bare_marker_r30.py` | T4-4 | R30 단위테스트 9개 |
| 이 문서 | 전체 | 설계 |
