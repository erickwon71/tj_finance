...
> 상태: **버그① Phase 1(구현+실측) 완료 — 코드/테스트 커밋 대기. 버그②는 여전히
> Layer 2 무죄 확정 + 잘못된 가설 소거까지만 완료(추가 원인추적 필요, §2-3).**
> 버그①의 **소급 백필(§3 체크리스트 ②)은 아직 미실행** — DB에 쓰는 작업이라
> 별도 명시적 승인 필요(`CLAUDE.md` "계획 후 대기" / 액션 확인 원칙).
>
...
>
> **★버그① Phase 1 실측 결과(2026-08-23, 같은 세션)** — §1-5 구현대로 진행하다
> **설계 문서 자체의 결함 2건을 실행/원문 대조로 발견해 그 자리에서 수정**했다
> (아래 §1-7). 요약: fy≥2024 tax_expense fail_a 89건 전수 재실행 —
> **85/89(95.5%) 적용, 오탐(applied_mismatch) 0건**, 미적용 4건은 전부 "행
> 자체가 텍스트추출에서 통째로 누락"(이 버그와 다른 별개 증상, §1-7-3). pytest
> 603 passed(무관한 기존 실패 1건 `test_biz_section.py::test_lxintl_facility_
> table_dropped` — base 브랜치에서도 동일 실패 확인, 이번 변경과 무관).
> 소급 백필(§3 체크리스트 ②)만 남았다 — 사용자 승인 후 진행.
>
> **★다음 세션(또는 이어서) 시작점**:
> - **버그①은 백필만 남았다** — `docs/runbook_new_parser_pipeline_integration.md`
>   체크리스트 ②(fy≥2024 전체 재적재 + std_v3 재빌드) + ③(Gate B 표본 재감사).
...
- [ ] **② 소급 백필** — fy≥2024 전체 재적재(`load_report_lines.py` 류) + `std_v3`
      재빌드. 정확한 범위(rcept 목록)는 Phase 1 구현 시 재산정. **미실행 — 사용자
      승인 대기**(DB 쓰기 작업).
- [x] **③ 검증(부분)** — §1-6/§1-7: 단위테스트 7건 신설 통과, fy≥2024 tax_expense
      fail_a 89건 전수 재실행(85 적용·오탐0·4 별개증상), pytest 전체 603 passed(무관
      기존실패 1건 확인). **Gate B 표본 재감사는 아직** — 백필 이후에나 의미가 있다
      (백필 전엔 std_v3가 그대로라 Gate B 판정도 안 바뀜).
