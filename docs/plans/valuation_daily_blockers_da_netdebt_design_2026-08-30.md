...
| 4 | **B1-D2(순서4)** — 원인C 해소, 하위 3단계로 확정 실행(§2-7): **①wiring**(orphan 3개 canonical을 net_debt 합산에 연결) → **②alias 3종**(신규 canonical 포함) → **③나머지 소수 라벨** | 잔여 미매핑 + 세부항목 미합산(원인C, §2-6) | 단계별 저→중위험 → 매 단계 4-4 전수재감사 | **①②③ 전부 완전 종료(2026-08-31)** — ①wiring(R58, `640f062`): net_debt 67.0%→56.2%/70.9%→59.9%. ②alias 3종(`7db4a55`): net_debt **56.2%→38.4%/59.9%→48.8%**(도약폭 최대). ③신주인수권부사채/교환사채(`4945ed1`+R57류 신규버그 수정`66c0f4e`): 재감사 2회(pass→fail_a 0×2) 끝에 net_debt 40.6%/51.4% — v2 비교 지표 자체가 v3 완전성 개선을 반영 못 하는 한계 도달(§2-10 원인규명 완료, 지표 악화 90%가 v2의 불완전성 노출이지 v3 결함 아님). 부산물로 별개의 진짜 버그(R59 후보, `bs.long_term_debt` rollup-라벨 HELD, `00181712` 62.9조 소실) 발견·규모실측(538건)까지 완료, 구현은 범위 밖(§6). |
| 5 | matview `net_debt`도 v3로 → **단일 LATERAL 복귀** | 이식 완료 | 낮음 | ✅ 완료(2026-08-31, 마이그레이션 `2026_08_valuation_daily_v3_netdebt_migration` + refresh, 상세: `docs/plans/valuation_daily_order5_netdebt_v3_migration_2026-08-31.md`) |
| 6 | **B2 수정**(`fin2/extract/notes.py`) | 1이 끝났으면 **불필요할 수 있다**(§B2-D4) — 그때 재판단 | 낮음 | ✅ 완료(2026-08-31) — **재판단 정정**: 1번(`bd39d44`)은 std_v2 재전파만 끊었을 뿐 `fact_v2` upsert(`store_facts`)는 그대로 살아있어 "신규 오염 이미 멈춤"은 부정확했음(코드 재확인으로 발견). 실제로 `extract_note_da_facts()`에 필터 추가 + 기존 합성행 2,597건 삭제까지 완료. 상세: `docs/plans/factv2_synthetic_da_total_cleanup_2026-08-31.md` |
...
### 2-9. ★★★★★순서4-② "alias 3종" 구현+전수재감사+원문대조 전부 완료(2026-08-31)

`account_maps/bs_accounts.py`에 alias 3종 등록(커밋 `7db4a55`, 상세는
`docs/PARSING_RULES.md` R58 순서4-② 절): `유동차입금(사채포함)`→
`bs.short_term_debt`, `비유동차입금(사채포함)의유동성대체부분`→
`bs.current_portion_lt_debt`, `전환사채`류→신규 `bs.convertible_bond`/
`bs.current_convertible_bond`(2개로 분리 — 실측상 `사채`+`전환사채` 동시
등장 231건, 유동성장기부채류+유동전환사채류 동시등장 1,300건이 있어 하나로
합치면 `_resolve()` 충돌로 오히려 손실). `_V3_ST_DEBT_PARTS`/
`_V3_LT_DEBT_PARTS`에 편입. 단위테스트 6건, `pytest` 651 passed/1
failed(기존 무관).

**전수재감사(`scripts/run_step2_alias_verification.sh`, 107분)**: std_v3
전체 재빌드 → Gate B 전수재감사 → **pass→fail_a 전이 0**(순서4-①과 동일
분포 — 안전성 재확인). `net_debt` v2/v3 불일치율: FY2024 **56.2%→38.4%**,
FY2025 **59.9%→48.8%** — 지금까지 중 최대 개선폭, "몸통 3개 라벨군" 가설
확정.

**원문 대조 2건**: `00100601`(alias 2종이 합산돼 v2와 원 단위까지 일치) ·
`00103130`(교환사채 미등록분을 제외한 나머지가 정확히 맞아떨어짐 — 순서4-③
필요성을 실측으로 재확인). 상세는 R58.

★순서4-② 완전 종료. 다음은 순서4-③(신주인수권부사채/교환사채) — 데이터
실측(2026-08-31) 결과 반드시 4개 canonical 신설 필요: `bs.exchange_bond`/
`bs.current_exchange_bond`(교환사채, 비유동/유동), `bs.warrant_bond`/
`bs.current_warrant_bond`(신주인수권부사채, 비유동/유동). 근거 — 같은
필링 내 동시등장(FY2022+ BS, table_seq=0 실측): `사채`↔`교환사채` 35건,
`사채`↔`신주인수권부사채` 30건, `전환사채`↔`교환사채` 139건,
`전환사채`↔`신주인수권부사채` **434건**, `교환사채`↔`신주인수권부사채`
37건, `교환사채`(비유동)↔유동 변형 63건, `신주인수권부사채`(비유동)↔유동
변형 88건 — 기존 canonical(`bond`/`convertible_bond` 등)에 합치면 전부
...
**★2차 전수재감사 — pass→fail_a 0(안전), net_debt 지표는 거의 무변화**
(40.5%→40.6%/51.2%→51.4%). 방금 고친 HELD-충돌은 사라졌는데도 지표가 안
움직인 이유를 끝까지 추적(사용자 지시) — **결론: 지표 악화의 90%는 버그가
아니라 v3가 v2보다 완전해진 것**(FY2024 mismatch 1,304건 중 1,169건이
v3>v2 방향, 그중 910건에 이번에 새로 배선한 라벨이 실제 존재. `00103130`
원문대조: 5개 실제 debt 라인 중 v3는 5개 다 잡고 v2는 2개만 잡음 — v2의
불완전함, v3 결함 아님, §2-7의 "v2=정답 전제 위험"이 대규모로 재확인됨).
**★단, 조사 중 완전히 별개의 진짜 버그를 하나 더 발견** — `bs.long_term_debt`
본체가 "차입금및사채"류 결합 라벨의 유동/비유동 동시존재로 HELD되는 R57류
버그(`00181712`: 62.9조 소실, 전수 538건). 순서1~3이 만든 게 아니고
DIRECT_MAP 핵심 컬럼이라 파급범위가 달라 별도 트랙(가칭 R59)으로 분리—
상세는 `docs/PARSING_RULES.md` R58 및 §6.
...
- **★(가칭 R59) `bs.long_term_debt`/`bs.short_term_debt` 본체의 rollup-라벨
  ...
  "사채및장기차입금" 등, 단일 개념이 아니라 사채+차입금을 합친 결합 라벨)가
  `bs.long_term_debt`에 fuzzy(0.95~0.97)로 매핑되는데, 이 결합 라벨이 같은
  필링 안에서 **유동부채 소속과 비유동부채 소속으로 동시에** 등장하면
  (같은 정규화 텍스트, 다른 section) `_resolve()`가 충돌로 HELD — R57/
  순서4-③이 고친 것과 **완전히 같은 병리**(만기 불분명 결합 라벨의 유동/
  비유동 동시존재)인데 `bs.long_term_debt`는 그 어느 가드
  (`_CURRENT_STRICT`도 `_CURRENT_CONTAMINATED_NONCURRENT_SIBLING`도)의
  보호 대상이 아니다. 실측 재현(`00181712` FY2024연결): `사채 및
  장기차입금`(14,788,886,000,000, 유동) + `사채및장기차입금`
  (48,073,129,000,000, 비유동) 동시존재 → `long_term_debt` 통째로 `None`,
  **62,861,073,000,000 소실**(이번 세션 발견 중 금액 최대). 전수 스캔
  (2011~2025, `report_lines` FY BS): "차입금및사채"류 결합 라벨이 유동+
  비유동 양쪽에 동시 존재하는 필링 **538건**.
  ★**순서1~3이 만든 버그가 아니다** — 관련 alias(`장기차입금및사채` 등)는
  이번 세션에서 손대지 않은 훨씬 이전 등록분. 다만 `bs.long_term_debt`/
  `bs.short_term_debt`는 net_debt 전용이 아니라 std_financials_v3의 핵심
  DIRECT_MAP 컬럼(다른 소비자 있음)이라 고치면 파급범위가 순서4-③의
  net_debt-scoped 우회보다 훨씬 크다 — 별도 설계·전수재감사 트랙(가칭 R59)
  으로 분리 권고. 지금은 발견·규모실측까지만, 구현은 미착수.
