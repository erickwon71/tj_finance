...
**행 안에 짝이 없는 잔여**: 트리니티항공(3셀)·핸즈코퍼레이션(4셀)은 정확한 값이 반대
basis 표에 있거나 합계 항등식으로만 확정된다 — 이 수정으로는 복원되지 않는다(왜곡값이
그대로 남는다). 합계 항등식 기반 2단계 복원은 미구현.
...
**등재 조건**(사용자 확정): 정정값이 **원문 다른 곳에 그대로 인쇄돼 있어** 추측 없이
확정될 때만. 근거를 주석에 반드시 남긴다(회귀 테스트가 항목마다 근거 주석 존재를
검사한다).
