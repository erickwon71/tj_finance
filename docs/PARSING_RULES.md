...
## R162. **[사용자 판단 대기 — 착수 금지]** SCE 표 원문에서 음수 괄호가 빠져
## 자본변동표 일부 셀이 부호 반대로 적재된다 (2026-09-22, 캠페인 이슈#28)
...
| 안 | 내용 | 대가 |
|---|---|---|
| 가 | 원문충실 유지(현행) | SCE 를 쓰는 분석을 나중에 만들면 그때 부호가 틀림 |
| 나 | 열 롤포워드 항등식으로 **자기증명되는 셀만** 부호 복원 (R154 와 같은 자기증명 패턴) | 탐지기 구현 + 전수 백필. 항등식이 닫히지 않는 표는 손대지 않음(R6) |
...
| 항목 | 상태 |
|---|---|
| **R162 — SCE 표 원문에서 음수 괄호가 빠져 자본변동표 일부 셀이 부호 반대로 적재됨(2015+ 1,209필링/2,552셀)** | **사용자 판단 대기 — 착수 금지**. 파서 결함이 아니라 DART 원문 렌더링 결함(BS/IS 는 괄호, 같은 필링 SCE 는 괄호 없음 — 효성중공업 `20190515002585` 실측). 참값은 열 롤포워드 항등식으로 확정 가능. **계층3 영향 0** — SCE 는 `_FS`(combine.py:1757)에도 없고 `_map_rows()` 가 걸러내므로 애초에 계층3 입력이 아니다. 선택지 (가) 원문충실 유지 (나) 자기증명 셀만 부호 복원(R154 패턴). 상세 = R162 |
