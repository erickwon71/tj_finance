#!/bin/bash
# R59(bs.long_term_debt rollup-라벨 유동/비유동 동시존재 시 HELD, fin2/layer3/combine.py)
# 검증 파이프라인을 한 번에 순차 실행한다: face_audit 스냅샷 -> std_financials_v3 전체
# 재빌드(5-shard) -> Gate B 전수재감사(5-shard) -> pass->fail_a 전이 확인.
#
# docs/plans/r59_rollup_debt_label_held_bug_design_2026-08-31.md §4
#
# ★face_audit은 long_term_debt/short_term_debt를 직접 감사하지 않는다(실측 2026-08-31 —
# fail_fields에 등장하는 필드 23종에 두 컬럼 모두 없음). 그래서 Gate B 회귀 확인은
# R57/R58과 똑같이 "전체 pass->fail_a 전이"만 본다(canonical 스코프 필터는 애초에
# 불가능 — face_audit에 그런 컬럼이 없음). 대신 이 수정 자체가 겨냥한 값 복구는
# std_financials_v3의 long_term_debt/short_term_debt를 직접 전/후 스냅샷 비교로 본다
# (0단계/5단계).
#
# 각 단계는 이전 단계가 전부 끝난 뒤에만 시작한다(gateb_audit 샤드는 재빌드가 끝난
# std_financials_v3에서 corp 목록을 다시 뽑아야 하므로 순서가 중요함).
#
# 사용법(터미널을 열어두고 지켜볼 때):
#   caffeinate -i bash scripts/run_r59_verification.sh
#
# 사용법(백그라운드로 돌리고 터미널을 닫아도 될 때):
#   caffeinate -i nohup bash scripts/run_r59_verification.sh > logs/run_r59_verification.log 2>&1 &
#
# 진행상황 확인:
#   tail -f logs/run_r59_verification.log
...
#   (재실행 시점엔 이미 재빌드가 진행 중이었을 수 있어, 다시 뜨면 베이스라인이
#   오염되기 때문).
...
SNAP_TABLE="face_audit_snap_20260831_r59"
N_SHARDS=5
YEAR_MIN=1999
...
DEBT_SNAP_TABLE="std_v3_debt_snap_20260831_r59"
echo ""
echo "===== 0단계: 영향 필링(rollup 라벨 유동+비유동 동시존재) std_v3 debt 컬럼 스냅샷 (${DEBT_SNAP_TABLE}) ====="
DEBT_SNAP_EXISTS=$(psql "$DB_URL" -tAc "SELECT to_regclass('public.${DEBT_SNAP_TABLE}') IS NOT NULL;")
if [ "$DEBT_SNAP_EXISTS" = "t" ]; then
    echo "스냅샷 테이블이 이미 있습니다 — 기존 베이스라인을 그대로 재사용합니다: ${DEBT_SNAP_TABLE}"
else
    psql "$DB_URL" -v ON_ERROR_STOP=1 -c "
    CREATE TABLE ${DEBT_SNAP_TABLE} AS
    WITH rows AS (
      SELECT corp_code, report_fiscal_year AS fiscal_year, basis,
             regexp_replace(label_raw, '\s+', '', 'g') AS norm_label,
             (section_path LIKE '%비유동%') AS is_nc
      FROM report_lines
      WHERE statement = 'BS' AND report_fiscal_period = 'FY' AND table_seq = 0
        AND (label_raw LIKE '%차입금%사채%' OR label_raw LIKE '%사채%차입금%')
        AND section_path LIKE '%유동%'
    ),
    affected AS (
      SELECT DISTINCT corp_code, fiscal_year, basis
      FROM rows
      GROUP BY corp_code, fiscal_year, basis, norm_label
      HAVING count(*) FILTER (WHERE NOT is_nc) > 0 AND count(*) FILTER (WHERE is_nc) > 0
    )
    ...
    "
    echo "스냅샷 생성 완료: ${DEBT_SNAP_TABLE}"
fi
psql "$DB_URL" -c "SELECT count(*) AS affected_filings, count(*) FILTER (WHERE long_term_debt_before IS NULL) AS held_before FROM ${DEBT_SNAP_TABLE};"
...
if [ "$fail" -eq 1 ]; then
    echo "std_v3 재빌드 중 하나 이상의 샤드가 실패했습니다. 로그를 확인하세요: logs/build_std_v3_shard_*.log"
    exit 1
fi
echo "std_v3 재빌드 ${N_SHARDS}개 샤드 전부 정상 종료됨. 로그 마지막 줄:"
tail -n 3 logs/build_std_v3_shard_*.log
...
if [ "$fail" -eq 1 ]; then
    echo "Gate B 전수재감사 중 하나 이상의 샤드가 실패했습니다. 로그를 확인하세요: logs/gateb_shard_*.log"
    exit 1
fi
echo "Gate B 전수재감사 ${N_SHARDS}개 샤드 전부 정상 종료됨. 로그 마지막 줄:"
tail -n 3 logs/gateb_shard_*.log

echo ""
echo "===== 4단계: pass -> fail_a 전이 확인 (Gate B 전체 — long_term_debt/short_term_debt는"
...
echo ""
echo "===== 5단계: long_term_debt/short_term_debt 복구율 (전/후 비교, ${DEBT_SNAP_TABLE}) ====="
psql "$DB_URL" -v ON_ERROR_STOP=1 -c "
SELECT
  count(*) AS affected_filings,
  count(*) FILTER (WHERE d.long_term_debt_before IS NULL) AS held_before,
  count(*) FILTER (WHERE d.long_term_debt_before IS NULL AND v.long_term_debt IS NOT NULL) AS recovered_after,
  count(*) FILTER (WHERE d.long_term_debt_before IS NULL AND v.long_term_debt IS NULL) AS still_held_after
FROM ${DEBT_SNAP_TABLE} d
...
echo "종료 조건: 4단계 pass_to_fail_a = 0 AND 5단계 recovered_after > 0(still_held_after는"
echo "0이 아니어도 됨 — sibling 이미 후보 존재 guard로 부분복구만 되는 케이스가 설계상 있음,"
echo "§3-3 참고. still_held_after 표본을 원문대조로 확인할 것."
