...
#
# ★ 2026-09-07(account_mapper 전체어휘 스캔, "취득/처분" 반의어쌍 갭, R80): CF/NOTE
# 섹션에 "취득"(자산증가·현금유출) canonical 만 있고 그 반의어 "처분"(자산감소·
# 현금유입) 전용 canonical 이 아예 없어 부호반대 계정으로 fuzzy 오매핑되던 6종을
# 처음엔 여기 전부 방어 등록했었다(무매핑이 부호오염보다 낫다는 원칙, §8-12 후속).
# 곧이어(R81) 사용자 지시로 "처분 부분 추가" 진행 — 6종 중 5종(종속기업·관계기업·
# 공동기업·투자부동산·자기주식/주석 자기주식)은 신규 canonical 을 신설해 정확히
# 등록했으므로 여기서 뺐다(account_maps/cf_accounts.py·note_accounts.py 참고).
# "기계장치의취득"/"차량운반구의취득" 2개만 계속 차단 유지 — 대응하는 처분쪽이
# 이미 cf.ppe_proceeds_detail(개별자산 처분 상세)에 등록돼 있는데, 취득쪽은 총계
# 라인(유형자산의취득→cf.capex)이 보통 같이 찍혀 있어 세부항목까지 별도 canonical
# 로 잡으면 총계와 중복계상될 위험이 커 **의도적으로 무매핑 유지**(임시방편 아님).
_FUZZY_BLOCK: set[str] = {
    "기계장치의취득", "차량운반구의취득",
}
