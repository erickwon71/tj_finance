...
### 11.1 파일럿 결과 (2026-09-24 13:12~15:47, run 17~46, 30회)

| 항목 | 값 |
|---|---|
| 결과 | passed 19 · has_issues 11 · error 0 |
| 시간 | 평균 4.6분, 중앙값 4.0분, 최대 11.3분 |
| 턴 | 평균 41, 최대 95(상한 120 안) |
| API 환산 | 평균 $1.57/회, 합계 $47.1 |
| 계정 사용률 | 주간 54→58%(회당 ≈0.13%p), 5h 회당 ≈1.6%p(같은 계정의 이 세션 사용분 포함 — 상한값) |
| 신규 실결함 | EPS 당기 공란에 전기 값 유입(#52~55), SCE 기초자본 행에 기말 값 유입(#56~58) 등. 이관 이슈 재확인: close 2 · reopen 3 |
| 검증자 오판 | 표본 1건(run 28, 별도 BS 48항목) 직접 원문대조 → 전부 일치. 2차 표본 6슬롯은 아래 §11.2 — 오판 0 |

### 11.2 검증 품질 표본 2차 (2026-09-24 21시, Sonnet 유지 근거)

Sonnet 러너가 passed 로 판정한 슬롯 6개(필링 11건)를 **파서와 독립적인 기계 대조**로 다시 확인했다.
도구: `scripts/verification_audit_compare.py <rcept> <xml>`.
원문 XML 표를 행 순서(difflib)로 DB `report_lines` 와 맞춘 뒤 값·열 위치·행 결측·SCE 셀 누락을 센다.
불일치 후보는 모두 원문 셀을 직접 열어 판정했다.

| 슬롯 | 선정 이유 | run·턴 | 필링 | 행 | 셀 | 결과 |
|---|---|---|---|---|---|---|
| 한진칼 2018FY | 최다 턴 | 40·95 | 3 | 925 | 1,528 | 일치 |
| 키움증권 2020FY | 증권 | 53·69 | 3 | 1,059 | 1,491 | 일치 |
| 현대해상 2018FY | 보험 | 70·62 | 1 | 359 | 462 | 일치 |
| 대한전선 2024FY | 2024+ 서식 | 59·51 | 2 | 658 | 1,625 | 일치 |
| iM금융지주 2021Q1 | 은행지주·분기 | 82·51 | 2 | 734 | 1,168 | 값 일치. 0원 행 3개 누락(아래) |
| 이마트 2023FY | 대조군 | 95·37 | 1 | 352 | 632 | 일치 |

- **값 불일치 0 · 부호 0 · 열 오귀속 0 · SCE 셀 누락 0 · 당기 값이 있는 행 결측 0.** 검증자 오판 0건이다.
- 참고(결함 아님 수준): iM금융지주 CF 3행(`상각후원가측정 유가증권 관련손실`·`사업결합에 대한 대가`·별도 `선급법인세`).
  당기 세부열이 빈칸이고 합계열만 `-` 라서 DB 에 행이 없다. 같은 표에서 당기 첫 칸이 `-` 인 행은 0 으로 적재된다.
  값 왜곡이 없어 이슈로 등록하지 않았다. 반복되면 "0원 행 누락"을 이슈로 볼지 #46 과 함께 정책 질문 대상이다.
- 스크립트 오탐(판정 전 원인 확인 완료): 원문 라벨의 `&cr;`, 2단 라벨(`상위>하위`), EPS(원 단위) 배수, 연결·별도 동일 라벨 표 선택,
  셀 안 `&cr;` 뒤 금액, 당기 빈칸인 전기 전용 행(한진칼 42~43행, IFRS9 전환 전 계정).
- skip 7건은 러너 판정이 아니라 이관된 legacy 판정(첨부정정 본문=감사보고서 0행)이다.
- 결론: **러너 모델은 Sonnet 을 유지한다.** 누적 표본 7슬롯(run 28 포함)에서 오판 0이다. §11 예산 재계산은 필요 없다.
- 이 스크립트는 §2 후보 ①(기계 대조)의 원형으로 쓸 수 있다. 11건 기준 오탐 원인은 위 목록으로 모두 정리됐다.

**파일럿 중 고친 결함**
- Chrome 미연결(`--chrome`)
- 도구 제한 무력화(`dontAsk`)
- 탭 누적(러너가 AppleScript 로 정리)
- 화면 판독으로 턴 소진(JS 표 추출 스니펫)
- SCE CSV 열 이름 누락(`열=`)
- 원문 오타 오등록(복원 확인 시 pass 노트)
- 러너 자기갱신(`main()` + 재실행)
- 테스트가 실제 알림 발송

**튜닝 확정(Max x5)**: `pace_mode=on`
- 5h 는 70% 가드.
- 주간 상한 = 100 − 40×남은비율 − 5. 오늘은 약 66%, 리셋 직전 약 95%.

**처리량 기대치**
- 주간 60%(러너 몫) ÷ 0.13%p ≈ 주 460회 ≈ 하루 60~65슬롯.
- 시총 상위 200개사 잔여 약 6,500슬롯 → **약 3~4개월**. §2.5 의 "6개월" 추정보다 빠르다.
- 단, 이번 주는 이미 58% 를 써서 상한(66%)까지 약 60회만 남았다. 본격 속도는 9/29 리셋 뒤부터 나온다.
