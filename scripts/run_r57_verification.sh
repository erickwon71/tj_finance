#!/bin/bash
# R57(net_debt `_NONCURRENT_SIBLING` 재라우팅, fin2/layer3/combine.py) 검증 파이프라인을
# 한 번에 순차 실행한다: face_audit 스냅샷 -> std_financials_v3 전체 재빌드(5-shard) ->
# Gate B 전수재감사(5-shard) -> pass->fail_a 전이 확인.
#
# docs/PARSING_RULES.md R57 / docs/plans/valuation_daily_blockers_da_netdebt_design_2026-08-30.md §5 순서3
#
# 각 단계는 이전 단계가 전부 끝난 뒤에만 시작한다(gateb_audit 샤드는 재빌드가 끝난
# std_financials_v3에서 corp 목록을 다시 뽑아야 하므로 순서가 중요함).
#
# 사용법(터미널을 열어두고 지켜볼 때):
#   caffeinate -i bash scripts/run_r57_verification.sh
#
# 사용법(백그라운드로 돌리고 터미널을 닫아도 될 때):
#   caffeinate -i nohup bash scripts/run_r57_verification.sh > logs/run_r57_verification.log 2>&1 &
#
# 진행상황 확인:
#   tail -f logs/run_r57_verification.log
...
SNAP_TABLE="face_audit_snap_20260830_r57"
N_SHARDS=5
YEAR_MIN=1999
...
echo "위 pass_to_fail_a 가 0이면 R57 트랙 종료 조건 충족."
