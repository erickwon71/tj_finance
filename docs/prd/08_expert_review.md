# 08 · 전문가 리뷰 — 재무 데이터 팜 & 투자 발굴 시스템

> 작성일 2026-07-02. 4개 전문가 렌즈(프로젝트 종합 / DB 운용 / **데이터 무결성**★ / 데이터 표시)로 현
> 프로젝트를 점검한 리뷰 + 우선순위 로드맵. 현황 정찰 3건 + 2026 best-practice 웹조사 근거.
> 개별 개선 항목은 착수 시 별도 상세계획으로 분해한다. **원칙: 기존 엔진·감사 자산 재사용, 재작성 금지.**

## 목표(리뷰 기준)

DART 공시 수집 → 전 재무정보 추출·DB화 → **재무 데이터 팜** → 재무성과 평가·시장평가지표 →
**투자 기업 발굴**. 이 큰 목표를 기준으로 평가한다.

**현 상태 한 줄**: 파이프라인(수집→추출→표준화→달력화→주가결합)과 report==DB 감사(Gate A/B)까지
탄탄하게 구축됨. 남은 격차는 ① 운영 안전(백업)·성능 ② **무결성의 독립 교차검증·상시화** ③ 이미 DB에
있으나 화면에 안 쓰는 데이터 자산.

**실행 확정(2026-07-02)**: 무결성 P0는 **I1→I2→I3 순차 전부** 진행. 백업(D1)은 병행 권장.
...
4. **미래기간 유령행(참조무결성 위반)**: std_v2에 `period_end>오늘` 8행(예: 2026-09-30 Q3, 실매출값) +
   ...
   > now()` CHECK 없음. (직전 세션에서 phantom 달력분기는 수정·`diag_calendar_orphans.py` 상주화.)
