"""★★이중대체됨(2026-09-18) — 이 스크립트는 근본원인(R134, has_note_col
미배선) 수정 전 임시 수동패치였다. R134 수정 후 `recover_one()`재적재로 아래
5개 중 4개(이연법인세부채·보통주자본금·주식발행초과금 + 자동추출로 원래 있던
것들)가 복구됐고, 남은 1개(미처분이익잉여금)는 "계정지도가 의도적으로 제외"
때문이라 판단해 manual 삽입했었다. 그런데 그 판단 자체가 틀렸다 — R135(같은 날
...
`unit_source='pdf'`). 이 파일은 R134/R135 발견 경위 기록용으로만 남겨두되,
**재실행하지 말 것**. 상세: `docs/PARSING_RULES.md` R134·R135.
