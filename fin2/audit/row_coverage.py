...
# ★날짜 칸은 금액이 아니다 — SCE 의 기초/기말 잔액 행과 '2015년 (제1기)' 같은 구간
#   표지 행은 라벨 옆 칸에 '2015.01.06' 을 적는다. 위 `_AMOUNT_LIKE_RE` 는 선두
#   '2015' 를 4자리 수로 보고 나머지를 `[\d,\.\s]*` 로 삼켜 금액으로 오인한다
#   (실측: 에브리봇 20170727000272 별도SCE `['2015년 \n(제1기)','2015.01.06',
#   '-','-','-','-']` — 금액이 하나도 없는 표지 행이 결측으로 발화). 금액다움을
#   보기 전에 날짜 모양을 먼저 배제한다.
_DATE_LIKE_RE = re.compile(
    r"^\d{4}\s*[.\-/년]\s*\d{1,2}\s*[.\-/월]\s*\d{1,2}\s*[.일]?$")


def _looks_like_amount(cell: str) -> bool:
    """그 칸이 **금액**으로 보이는가(주석번호·날짜/기간표기 제외)."""
    s = (cell or "").strip()
    if not s:
        return False
