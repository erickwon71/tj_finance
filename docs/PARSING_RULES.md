...
**규모 — UTF-8 모집단에서는 이 변형이 이 1건뿐이다.**
SD 미러에서 `대손준비금|비상위험준비금` 을 가진 필링(2015+, 1,520건)을 수정 전/후로
재추출해 비교했다: **변화 0건**(`scripts/measure_r152b_dash_variant.py`).

★**단, 그 모집단 자체가 불완전했다** — `grep "대손준비금"` 은 **EUC-KR 필링을 통째로
놓친다**(그 바이트열이 파일에 없다). 정작 당사자인 우리금융지주 `20200330004490` 이
목록에 없었다. 두 인코딩 패턴으로 다시 훑는 중이다
(`LC_ALL=C grep -arlF -f <utf-8 + euc-kr 패턴>`). 교훈은
[[feedback-grep-euckr-locale-trap]] 에 보완했다 — **프리필터를 만들면 이미 아는 양성
사례가 목록에 있는지 먼저 확인할 것.**
...
## R155. **[수정 안 함 — 알려진 한계]** 구형 2단 열(내역칸/잔액칸) 서식에서 양쪽이
...
## R166. 라벨 칸이 ROWSPAN 으로 이어지는 **두 번째 물리행**의 값이 통째로 버려졌다

**발견.** 캠페인 이슈#34(camp_run) — SK이노베이션 2023FY `20240320000950` [연결] SCE,
라벨 `기타포괄손익-공정가치측정 금융자산 평가손익` 의 3개 연차 구간에서 값이 결측.

**원인.** DART 는 라벨이 길면 라벨 `<TE>` 에 `ROWSPAN=2` 를 주고 **값을 둘째 물리행에
싣는다**. 그 행에는 자기 라벨 칸이 없으므로 `_grid_body_rows` 의 `physical[0]` 이 첫
**금액** 칸이 되고, 그 텍스트가 비어 있으면

```python
if not label:
    continue        # ← 행 전체가 값째로 버려진다
```

에 걸린다. 상속 칸은 origin 텍스트를 이미 들고 있으므로 라벨은 복원 가능한데도 보지 않았다.

**★세 구간이 같은 모양이 아니다** — camp_run 이 스크린샷 + computed style 로 확인한 바
**화면과 XML 은 일치**한다(라벨 셀만 height 65px = 2행분, 나머지는 32px 단일행). 렌더
괴리가 아니라 원문 레이아웃이다.

| 구간 | 라벨행(TR) | 둘째 행 | 판정 |
|---|---|---|---|
| 2022 `TR24/25` | **전부 공란** | 4열 값, 그 행만으로 항등식 닫힘 `(48,279,660)+(4,822,811)=(53,102,471)` | 병합이 옳다 |
| 2023 `TR42/43` | **전부 공란** | `28,480,153 + (1,233,268) = 27,246,885`, `+1,360,315 = 28,607,200` | 병합이 옳다 |
| 2021 `TR6/7` | **이미 자기완결** `20,388,640 + 17,913 = 20,406,553` | 자본합계 칸에 `20,599,397` 하나뿐 — **어떤 항등식도 닫지 않음** | ★정체 불명 |

**수정.** 라벨 영역(`grid_col < offset`)에 **물리 셀이 하나도 없는** 본문 행을 직전 논리행의
**이어짐**으로 보고, **그 행의 빈 열만 채운다.** 이미 값이 있는 열은 건드리지 않는다.

- 2022·2023 은 라벨행이 비어 있어 전부 복구된다.
- 2021 은 열이 이미 차 있어 **자동으로 거부**된다 — 정체를 모르는 값을 밀어넣지 않는다(R6).
- **새 행으로 쪼개지 않는다**: 같은 라벨의 행이 둘이 되면 열 롤포워드 항등식(R162·R165)이
  깨진다.

**검증.** 그 필링 [연결] SCE 셀 **211 → 220**, 세 구간 **전부 항등식 닫힘**(2022·2023 은
원래 행 자체가 없었다). `20,599,397` 은 **복구되지 않는 것이 정답**이다. 회귀 테스트 4건
(합성 3 + 실측 1), 변이 검사로 비침묵 확인(수정을 되돌리면 2건 실패 — 그 중 하나는
"정체 불명 값이 주입되면 실패" 하는 오염 감시 테스트다).
`pytest tests/ fin2/tests/` — 1,289 passed.

★**내가 한 번 잘못 짚고 되돌렸다**: 처음엔 "라벨이 두 줄로 렌더돼 나머지 열이 둘째 줄로
밀린 것" 으로 보고 병합을 넣었는데, 2021 구간의 라벨행이 **이미 완결**(항등식이 닫힘)인 것을
확인하고 "한 논리행이 쪼개진 것" 이라는 전제가 틀렸다고 판단해 **커밋 전에 되돌렸다**.
그 뒤 camp_run 의 화면 대조로 2022·2023 은 실제로 라벨행이 비어 있음이 확인돼 다시 넣었다.
같은 결함처럼 보이는 세 구간이 실은 두 가지였다 — "같은 증상 ≠ 같은 원인" 의 또 다른 사례.

### R166-b — 라벨행이 **명시적 `0`** 으로 채워진 변형(캠페인 이슈#35)

**증상.** 한미반도체 2023H1 `20230814001921` [연결] SCE `자기주식처분이익` 행 결측.

**원인.** R166 은 "직전 행이 이미 값을 가진 열은 건드리지 않는다" 로 막았는데, 원문이
**빈 칸 대신 `0` 을 찍는** 서식이 있다. 그러면 라벨행의 모든 열이 '이미 찬 것' 이 되어
이어짐 행의 실제 값이 **전부 거부**된다.

```
row17 (라벨행)   자기주식처분이익   전 열이 명시적 '0'
row18 (이어짐)   (상속)            자본잉여금·지배기업지분합계·자본합계 = 5,464,171,128
```

**참값 근거 2개.** ①같은 필링 **[별도] SCE** 에 같은 금액이 **단일 행으로 정상 적재**돼
있다(SCE 밖은 아니지만 이 결함 경로 밖의 독립 관측이다). ②이어짐 행 자체의 항등식이 닫힌다
(`0 + 5,464,171,128 = 5,464,171,128`).

**수정.** **직전 값이 `0` 이고 들어오는 값이 `0` 이 아닐 때만** 대체한다. 0 은 자리표시로
보고, 그 외의 실제 수치는 종전대로 절대 덮지 않는다 — SK이노베이션 2021 구간(직전 값이
20,406,553 등 실제 수치)은 **그대로 거부**되어 `20,599,397` 주입 금지(R6)가 유지된다.

★같은 필링 안에 **방향이 반대인 두 케이스가 공존**한다(2022 구간은 라벨행이 실값·이어짐행이
전부 0). 그래서 "라벨행이 먼저냐 나중이냐" 같은 위치 규칙으로는 못 가르고, **값이 0 인가**로
갈라야 한다. camp_run 의 가설이 그대로 맞았다.

**검증.** [연결] 3셀 복구(자본잉여금·지배기업지분합계·자본합계), [별도] 2셀은 원래 정상이라
불변. 회귀 테스트 4건 추가(합성 3 + 실측 1), 변이 검사로 비침묵 확인(R166-b 만 되돌리면
2건 실패). `pytest tests/ fin2/tests/` — 1,293 passed.

**전수 규모: 미측정.** `_grid_body_rows` 는 SCE·주석이 공유하는 경로라 영향 범위를
2015+ 전수 표귀속 diff 로 재야 한다(R164-b 와 같은 방식). 별도 측정 예정.
...
| **R166 — 라벨 ROWSPAN 둘째 물리행의 값이 통째 유실** | **✅수정 완료 · 전수 규모 미측정(2026-09-23)**. 라벨 `<TE ROWSPAN=2>` 뒤 둘째 행은 자기 라벨 칸이 없어 `physical[0]` 이 첫 금액 칸이 되고, 빈 텍스트가 `if not label: continue` 에 걸려 행 전체가 값째로 버려졌다. 발견=캠페인 이슈#34(SK이노베이션 `20240320000950` [연결] SCE, camp_run 화면대조로 렌더 괴리 아님 확인). 수정=**직전 논리행의 빈 열만** 채움 — 2022·2023 구간은 라벨행이 공란이라 복구되고, 2021 구간은 라벨행이 이미 자기완결이라 자동 거부(그 `20,599,397` 은 어떤 항등식도 닫지 않아 **정체 불명 → 넣지 않는다**, R6). **R166-b**: 라벨행이 빈 칸 대신 명시적 `0` 인 변형(한미반도체 `20230814001921`, 이슈#35)은 **직전 값이 0 일 때만** 대체 — 0 이 아닌 실제 수치는 계속 불가침. ★`_grid_body_rows` 는 SCE·주석 공용이라 2015+ 전수 표귀속 diff 필요(R164-b 방식) — **미측정** |
