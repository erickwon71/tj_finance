...
## 1-1. ★야간 결과 (2026-09-25 03:15) — 아침에 먼저 볼 것

**기계 대조 1회전 + mc5 재대조 완료** (설계 `docs/plans/verification_machine_compare_design_2026-09-24.md` §6~8)

| 항목 | 전날 21시 | 03:15 |
|---|---|---|
| 슬롯 passed | 1,695 | **70,541** |
| 슬롯 has_issues | 26 | 15,568 |
| 모델 대기 pending | 92,856 | **8,468** |
| open 이슈 | 70 | 83,151(기계 자동 82,567 · 16,888필링) |

- 도구 개선: mc1 → mc5 (SCE 항등식 설명·자동 부호이슈, 명세 표 제외, 자본과부채총계, 반복 라벨 재짝짓기, zero_row 정보성).
- 모델 러너 흐름(`machine.gate=on`)
  - 기계가 넘긴 발견만 웹뷰로 확인한다. 회당 1~10분이다.
  - 발견이 많으면 표본 3건을 확인한 뒤 `machine issues-json` 으로 일괄 등록한다(run 105 의 120턴 초과 이후 추가).
  - 재확인 슬롯은 close/reopen 만 하고 나머지는 기계에 넘긴다.
- 모델 대기 8,468 슬롯: 구성은 불일치, 원문 XML 없음 1,627필링, audit 210필링. 예산 페이스로 하루 약 60~100 슬롯이면 **약 3개월**이다. 추가 단축은 아래 판단 뒤에 한다.

**★사용자 판단 필요**
1. ✅(05:10 완료 — 사용자 지시로 R162-e 배치 #2 진행: 재적재 16,888 · closed 70,719 · reopened 11,848, 슬롯 passed 79,344 · 모델 대기 11,651. PARSING_RULES R162-e) **기계 자동 이슈 82,567건** (sign_flip, rule R162).
   - 원문이 괄호를 빠뜨린 음수 셀을 SCE 롤포워드 산수로 증명한 것이다. 한 셀만 뒤집어야 닫히고 다른 후보가 없는 경우만 등록했다.
   - 대부분 **배당금 행을 양수로 공시하는 관행**이다(배당 관련 약 5.4만).
   - 원문으로 표본 확인했다(20260313000835 현금배당 15,182,251,900, 20180525000233 배당금지급).
   - 수정 쪽 R162 복원을 "BS/IS 짝이 없는 행(배당 등)은 롤포워드 단독 증명" 으로 넓히는 배치 하나가 될 성격이다. 이 규모로 진행할지 판단이 필요하다.
2. **zero_row(전열 `-` 행 누락)**: 기계는 기록만 하고 막지 않게 바꿨다(모델 대기 6,352필링 절감).
   - 캠페인은 전에 이슈로 등록했다(#103~).
   - 이슈로 올리려면 `vq.py machine issues-json --kinds zero_row` 를 쓴다.
3. 기존 legacy/모델 passed 1,695 슬롯을 기계로 재대조할지 여부.
