...
_NI_TOTAL_RE = re.compile(
    r"^당?(기|분기|반기)(?:\([^)]{1,6}\))?의?순(이익|손익|손실)|"
    r"^순(?:이익|손익|손실)의\s*귀속"
)
...
# ★R47-c 조사 중 추가 발견(2026-08-26) — 두 갈래 더 넓힘:
#   (a) 기간어와 순 사이에 괄호 별칭이 끼는 표기("당기(분기)순이익(손실)의 귀속", 심텍홀딩스
#       00152127 실측) — 위 `(?:\([^)]{1,6}\))?`.
#   (b) 기간어 자체가 아예 없이 "순이익의 귀속"만 쓰는 표기(삼영 00127255 실측: TOTAL 행은
#       "당기순이익(순손실)"로 정상 매치하지만, 그 뒤 실제 귀속 섹션 헤더는 별도로 다시
#       "순이익의 귀속"만 나옴 — 첫 매치 뒤 OCI 행들을 거쳐 "총포괄" CLOSE 로 섹션이 닫히고,
#       진짜 귀속 헤더는 그 뒤에 나와 원래 정규식으론 재오픈이 안 됐다). "의 귀속" 접미를
#       필수로 요구해 좁게 열어(법인세비용차감전순이익 등 상위 소계와 혼동 안 되도록) 안전.
