...
```
[완료] Phase 0  오늘 변경분 커밋 + 테스트                        (0d2bdc3, 253/253)
[완료] Phase 1  T1·T2·T3 작성·실행 (T4 pre-2015 만 미실행)
[완료] 즉시     미사용 인덱스 drop — 실제 회수는 **680 MB**      (2026-07-31)
                ⚠ note_lines_corp_fy_basis_idx(1.5 GB)는 유지로 정정 — 원장 말미 참고
[완료] Phase 2  F1 단위 열귀속(681e42a) · F2 header_hint(14cd8e7, 계층3 가드 동봉)
[완료] (선행)   F4 데일리 본문 배선(a2bb0c7) — 재적재와 독립이라 먼저 했다
[완료] Phase 5  어서션 2종 선반영(3509fee) — unit_contamination 은 재적재 후 ERROR 로 승격
 ↓
[다음] ★사용자 결정 3건 → docs/qa/handoff_f1_f2_units_2026-07-31.md §4
[다음] Phase 3  신규 추출기 — T2 셀 수 순서로. 항목마다 용량 사전 추정
[다음] Phase 4  F3 스키마 축소 → **재적재 1회**(~5h)
별도결정 pre-2015 82,005건 (T4 결과 보고 후)
```

> ★2026-07-31 실행 메모 — 계획과 달라진 점 세 가지
> 1. **F4 를 Phase 4 에서 떼어 먼저 했다.** 재적재에 의존하지 않고(추출은 이미 하고 있었고
>    `store_report_lines` 호출만 빠져 있었다), P0 를 하루라도 빨리 닫는 편이 낫기 때문이다.
> 2. **인덱스 회수량이 2.2 GB → 680 MB.** `note_lines_corp_fy_basis_idx` 는 데일리 증분
>    적재가 쓰는데 야간 잡이 삭제돼 통계에 안 잡혔을 뿐이다(원장 정정).
> 3. **F1 의 범위가 넓어졌다.** T1 census 가 미선언 버킷을 '성격 미확인'으로 남겼는데,
>    확인해 보니 셀의 81%가 진짜 데이터표라 전사 대상에 포함했다(`value_won` 은 비운다).
