...
## 계층 3 — 취합 (★계층2 전량 DB 적재 완료 후 착수)

> **일정 확정(2026-07-19, 사용자):** 계층3 은 **계층2 가 실제 parsing 까지 끝나 report_lines
> 가 전량 DB 에 적재된 뒤** 시작한다. 파일럿·엔진 완성만으로는 착수하지 않는다 — **원문 tree 가
> DB 에 다 들어간 상태**를 입력 전제로 삼아, 취합 로직을 완성된 데이터 위에서 개발·검증한다.

- **입력**: `report_lines` tree (전량 적재 완료본).
...
## 진행 순서 (★2026-07-19 재정렬: 전량 적재 4패스 → 그 후 계층3)
1. **계층2 엔진**: `report_lines` 스키마 + 추출(엔진 재사용 + section_path + 주석 슬라이스). — ✅ 완료
2. **계층2 검증**: 파일럿(금융업 KG케미칼 포함) 원문 1:1 대조 통과.
3. **계층2 전량 적재 — 4패스**(순서대로, 각 패스 무결성 어서션 통과 후 다음):
   - **1차** 2015+ 원본 93,801(TrackA+B 한 번에) · **2차** pre-2015 원본 70,374(신규 파서)
   - **3차** PDF-only 3,575(Track C 어댑트, tree 품질 낮음) · **4차** 정정 처리 + **기간당 원문 1개 선택**
   ← **4패스 전부 완료 = 전량 적재 완료. 그때 계층3 착수**(사용자 확정 2026-07-19).
4. **계층3**: 정본 1개/기간 report_lines 위에서 취합(금융업 합산·capex·K-IFRS). → std_v2(version=2).
5. **검증(무결성 어서션) → swap → App 재연결.**

> ⚠ 구 순서 '계층3 → 전량' 을 **'전량(4패스) → 계층3'** 으로 뒤집음(사용자). 취합은 완성된 전량
> 데이터 위에서 개발해야 엣지·기업별 편차를 실데이터로 다룰 수 있음.
> ★ **정본선택(reconcile)을 계층2 4차로 이관** — "결국 원문 1개만 남아야"(사용자). 계층3 입력은
> 이미 기간당 1개로 정리된 상태. PDF-only는 당분간 보류 성격(3차), 정정은 4차로 미룸.

> **진행 추적 체크리스트 = `docs/plans/rearchitecture_4layer_2026-07-19_checklist.md`** (Phase별 ☐/◐/☑).
