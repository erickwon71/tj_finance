...
def test_loaded_position_is_found_by_value_not_by_first_number():
    ...
    **누적**을 적재한다(R144). '첫 금액 위치'로 잡으면 3개월 열(1)이 적재 위치로
    ...
    # 열마다 값이 다른 행은 매치가 유일 → 그 위치(누적 열)가 잡힌다.
    ...
    # ★Q1 은 3개월 == 누적 이라 적재값이 **두 칸에 모두** 나타난다 — 이런 행은
    #   어느 칸에서 적재됐는지 말해주지 못하므로 **투표에서 빼야** 한다. 안 빼면
    #   "먼저 만난 칸"(3개월)이 이겨서 누적이 빈 행이 결측으로 둔갑한다
    #   (실측 거짓양성: 제닉 20150515001151 '지분법 자본변동').
    ambiguous = [["수익(매출액)", "16,558,035,571", "16,558,035,571", "14,487,143,074"],
                 ["지분법 자본변동", "28,839,787", "", ""]]
    loaded = {row_coverage.normalize_label("수익(매출액)"): {16_558_035_571}}
    assert row_coverage._loaded_value_positions(ambiguous, loaded) == set()
