#!/bin/bash
# B1-D2 step2 "alias 3종" (account_maps/bs_accounts.py + fin2/layer3/combine.py) 검증
# 파이프라인 원샷: face_audit 스냅샷 -> std_financials_v3 전체 재빌드(5-shard) ->
# Gate B 전수재감사(5-shard) -> pass->fail_a 전이 확인 -> net_debt v2/v3 불일치율 측정.
#
# docs/plans/valuation_daily_blockers_da_netdebt_design_2026-08-30.md §2-7 순서2
#
# run_step1_debt_wiring_verification.sh 와 동일 패턴 — 스냅샷 테이블/로그 이름만
# step2 전용으로 분리(combine.py 자체가 또 바뀌므로 step1 스냅샷 재사용 불가).
#
# 사용법(터미널을 열어두고 지켜볼 때):
#   caffeinate -i bash scripts/run_step2_alias_verification.sh
#
# 사용법(백그라운드로 돌리고 터미널을 닫아도 될 때):
#   caffeinate -i nohup bash scripts/run_step2_alias_verification.sh > logs/run_step2_alias_verification.log 2>&1 &
#
# 진행상황 확인:
#   tail -f logs/run_step2_alias_verification.log
#   tail -f logs/build_std_v3_step2_shard_*.log
#   tail -f logs/gateb_step2_shard_*.log
...
SNAP_TABLE="face_audit_snap_20260831_step2"
N_SHARDS=5
YEAR_MIN=1999
...
python scripts/measure_net_debt_v2_v3_mismatch.py --label before_step2_rebuild | tee -a logs/net_debt_mismatch_progress.log
...
for i in $(seq 0 $((N_SHARDS - 1))); do
    python scripts/build_std_v3.py --all --year-min "$YEAR_MIN" \
        --shard "${i}/${N_SHARDS}" \
        > "logs/build_std_v3_step2_shard_${i}.log" 2>&1 &
    pids+=("$!")
...
if [ "$fail" -eq 1 ]; then
    echo "std_v3 재빌드 중 하나 이상의 샤드가 실패했습니다. 로그를 확인하세요: logs/build_std_v3_step2_shard_*.log"
    exit 1
fi
echo "std_v3 재빌드 ${N_SHARDS}개 샤드 전부 정상 종료됨. 로그 마지막 줄:"
tail -n 3 logs/build_std_v3_step2_shard_*.log

echo ""
echo "===== 3단계: net_debt 불일치율 측정(재빌드 후) ====="
python scripts/measure_net_debt_v2_v3_mismatch.py --label after_step2_rebuild | tee -a logs/net_debt_mismatch_progress.log
...
for i, s in enumerate(shards):
    with open(f"scratchpad/gateb_step2_shard_{i}.txt", "w") as f:
        f.write("\n".join(s) + "\n")
    print(f"scratchpad/gateb_step2_shard_{i}.txt: {len(s)}개사")
PYEOF

pids=()
for i in $(seq 0 $((N_SHARDS - 1))); do
    python scripts/gateb_audit.py --source v3 --fy-min "$YEAR_MIN" --recheck \
        --corp-file "scratchpad/gateb_step2_shard_${i}.txt" \
        > "logs/gateb_step2_shard_${i}.log" 2>&1 &
    pids+=("$!")
...
if [ "$fail" -eq 1 ]; then
    echo "Gate B 전수재감사 중 하나 이상의 샤드가 실패했습니다. 로그를 확인하세요: logs/gateb_step2_shard_*.log"
    exit 1
fi
echo "Gate B 전수재감사 ${N_SHARDS}개 샤드 전부 정상 종료됨. 로그 마지막 줄:"
tail -n 3 logs/gateb_step2_shard_*.log
...
echo "위 pass_to_fail_a 가 0이고, net_debt 불일치율이 개선됐으면 step2 트랙 종료 조건 충족."
