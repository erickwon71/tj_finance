...
| D3' | 미러의 `--delete` | **데일리에서 제거 — 덧붙이기(`rsync -a`) 전용.** 지우기는 별건(§5.3.1·§6.4b). ★2026-08-01: 그 별건(SD 폴더 제거)이 수동→**데일리 자동**으로 바뀜(§13). 전역 `--delete` 를 안 쓰는 것은 그대로. 근거: 상장폐지 10사 = 857MB(SD 여유의 0.6%)로 얻는 것이 없는데 유일한 치명적 연산 |
...
| 상장폐지 기업 폴더를 정리하나 | Phase 4 — 안전장치 통과 후 NAS 아카이브 이관 + SD 정리, **둘 다 데일리 자동**(§13, 2026-08-01) |
...
## 13. 결정 변경 — 상장폐지 원문 조치 자동화 (사용자, 2026-08-01)

**바뀐 것**: §6.4/§6.4b 에서 "수동"이던 두 조치를 **데일리 자동**으로 올린다.
결정 D1(원문 삭제 금지·NAS 아카이브 영구 보존)과 D3'(데일리 미러에 `--delete` 없음)는 그대로다 —
지우는 대상은 **SD 백업 폴더뿐**이고, 방식도 여전히 전역 `--delete` 가 아닌 명시적 폴더 목록이다.

| | 2026-07-31 계획 | 2026-08-01 부터 |
|---|---|---|
| NAS 아카이브 이관 | 수동 `--archive --apply` | **데일리 ⓪-4 자동** |
| SD 백업 폴더 제거 | 수동 `--sync-backup --apply`(연 1~2회) | **데일리 ⓪-4 자동** |
| 판정(⓪-3)과의 관계 | 판정만 자동 | 판정은 그대로 ⓪-3, 파일 조치는 분리된 ⓪-4 |

### 13.1 구현

- `collector/delisting_archive.py` **신설** — 이관·정리 로직 단일 구현.
  `scripts/delisting_manage.py` 는 이 함수를 부르는 수동 진입점으로 축소(자동/수동이 각각
  구현을 갖고 따로 굳는 것을 막는다).
- `scripts/collect_new.py` `_sync_delisting_archive()` = **⓪-4**. 호출 위치는 ⓪-3 직후,
  **⑥ 미러보다 앞**. 순서가 뒤바뀌면 방금 지운 SD 폴더를 미러가 다시 채운다.

### 13.2 자동화하면서 새로 넣은 가드

| | 내용 | 없으면 |
|---|---|---|
| A1 | 한 실행당 이관 상한 20개(= `DAILY_CONFIRM_CAP_REGISTRY`). 초과 시 **부분 실행이 아니라 전면 중단** + 메일 | 판정이 대량 오탐 나면 아카이브가 그만큼 번진다 |
| A2 | SD 삭제 전 `assert_storage(require_backup=True)` | 빈 마운트포인트를 SD 로 오인 → 백업 전멸(§5.3.1 의 그 함정) |
| A3 | SD 삭제 전 NAS 아카이브 **실재 + 파일 수 대조**(아카이브 < SD 면 보류·경고) | 이동이 덜 끝난 상태에서 백업만 지운다 |
| A4 | 전역 `rsync --delete` 금지 — 확인된 폴더만 `rmtree` | 계획 단계에서 이미 치명으로 분류된 연산 |
| — | 아카이브에 같은 이름이 있으면 덮어쓰지 않고 오류 | 과거 이관분 유실 |

### 13.3 같이 고친 것 — ⑦ 완전성 감사의 오검출

⑦ 의 "당일 다운로드분 파일 실재 확인"은 `completed` 인데 파일이 없으면 `pending` 으로 되돌린다.
아카이브가 자동이 되면 **어제 받고 오늘 폐지 확정된 기업**의 원문이 여기 걸려
정상 수집분이 미완료로 되돌아간다. `archive_path IS NOT NULL` 인 기업을 쿼리에서 제외했다.
(`audit_download_gap.py` 는 `is_active` 기업만 보므로 영향 없음 — 확인함.)

### 13.4 검증

- `tests/test_delisting_archive.py` 12건 신설(A1~A4·드라이런·덮어쓰기 금지·후보 불가침).
  결함 주입 방식: 아카이브 실물 제거·파일 수 부족·상한 초과. 전부 "백업이 지워지면 실패".
- 회귀 `tests/run_all.py` 95/95 · `pytest tests/` 142/142.
- 첫 실행 대상은 확정 12사 ≈ 1.1GB (상한 20 이하라 한 번에 끝난다).

---

## 14. 휴장일 스킵 (사용자, 2026-08-01)

**결정**: KRX 휴장일(주말·공휴일·임시휴장)에는 데일리를 **전면 스킵**한다.
정기보고서 접수·시세·상장/폐지는 전부 영업일에만 생기므로 휴장일 실행은 얻는 것 없이
KRX·DART API 만 헛친다.

### 14.1 왜 launchd 가 아니라 코드인가

plist `StartCalendarInterval` 의 `Weekday` 로는 **주말밖에** 표현할 수 없다. 공휴일은
어차피 코드에서 봐야 하므로 판정을 한곳(`collector/market_calendar.py`)에 둔다.
소스는 `exchange_calendars` 의 **XKRX** 달력(대체공휴일·임시휴장 포함, 상용 라이브러리가 유지).
하드코딩 공휴일 목록은 두지 않는다 — 반드시 낡고, 낡은 것을 아무도 모른다.

### 14.2 배선

- `scripts/collect_new.py` **⓪-0** — `--skip-holidays` 일 때만, **저장소 계약(⓪)보다 먼저**.
  안 돌 날이면 볼륨조차 건드리지 않는다.
- plist 에 `--skip-holidays` 추가(설치본 반영·bootstrap 확인 완료).
  **손으로 돌릴 때는 플래그를 주지 않으므로 휴일에도 그대로 작업할 수 있다.**

### 14.3 누락이 안 생기는 이유

스킵은 `pipeline_runs` 에 **기록을 남기지 않는다**. 워터마크가 마지막 성공 실행 그대로 유지되므로
다음 영업일 `--days auto` 가 건너뛴 구간까지 함께 회수한다(3일 겹침·90일 상한).

### 14.4 fail-open

달력 조회가 실패하거나(라이브러리 이상) 날짜가 수록 범위(현재 ~2027-07) 밖이면 **개장일로 보고
실행한다.** 비대칭이 명확하다 — 잘못 쉬면 그날 수집이 통째로 비고, 잘못 돌면 API 를 몇 번
헛치고 끝이다. 범위 밖은 경고 로그를 남긴다(`exchange_calendars` 업데이트 신호).

### 14.5 검증

- `tests/test_market_calendar.py` 7건(주말·공휴일·평일·fail-open 2종·다음 개장일).
- 실제 명령 실행: `--days auto --download-only --skip-holidays` → 2.5초 만에
  `⓪-0 2026-08-01 (토) 주말 — KRX 휴장. 다음 개장일 2026-08-03` 로 종료.
- 회귀 `tests/run_all.py` 102/102 · `pytest tests/` 149/149.
