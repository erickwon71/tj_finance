...
**규모(2015+ 전수 104,533필링, `scripts/measure_r166_rowspan_continuation.py`).**
트리거(라벨 영역에 물리 셀이 없는 본문 행) 기준:

```
영향 필링 55 · 트리거 행 188 · 원문손상 1(기존 triage 건)
  SCE_C 40필링 · SCE_S 15필링 · BS_C 1 · BS_S 1     ← 주석 표는 영향 없음
```

★이 측정은 **병합안 기준**으로 만든 것이라(빈 열 채움 197 / 0 대체 45 / 거부 532)
지금 설계의 산출과 1:1 대응하지 않는다. 지금 설계에서는 그 188개 트리거 행이 **각각
독립 행으로 추가**된다. 재측정이 필요하다 — 미완.
...
`pytest tests/ fin2/tests/` — 1,294 passed.
