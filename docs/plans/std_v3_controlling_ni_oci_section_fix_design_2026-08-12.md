...
### Phase 1 — 코드 구현 (다음 승인 대상)
- [ ] `_NI_ATTRIBUTION_CANON` 상수 추가.
- [ ] `_resolve()`에 §2-2 분기 삽입(stage-rank 숏컷 우회, 항상 conflicts로).
- [ ] 유닛테스트 작성(§4-2-1, 스왑 반례 포함).

### Phase 2 — 검증
- [ ] 404건 재현 스캔(§4-2-2), §3-1 예상치(301/321)와 대조.
- [ ] PASS 광역 재검증(§4-2-3) — 변경 156류를 원문대조로 표본 확인, 신규 회귀 여부 최종 판별.
