...
★★★★★**2026-09-08 같은 날 후속 — 카테고리1(트랙2, 49건) 전제 무효화됨.** 사용자가
표본(00104698)을 DART 원문과 직접 대조하자고 지적 → **DB에 저장된 값이 원문 재무제표
어디에도 존재하지 않음**(단순 배제 대상이 아니라 "존재하지 않는 숫자") 확인. 이건
R63류 stale-reprint 배제로 고칠 문제가 아니라 PDF복구 추출 파이프라인의 값 조작
버그다 — **트랙2(row 단위 배제) 설계는 보류**, 트랙1(전기FY 비교 arm, 카테고리2
11건)은 이 발견과 무관하므로 유효함. 상세: `docs/qa/report_lines_row_count_outlier_
scan_2026-09-08.md`.
