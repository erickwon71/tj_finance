...
| **R49(신규, R47 조사 중 발견) — std_v3 controlling_ni 에 총포괄손익 값이 잘못 채택된 stale 필링 2건(face_audit.py 스코프 밖)** | **미조치, 원인 가설 高확신·미검증** — 00201432(2025Q1)·00124504(2025FY) 둘 다 `is.controlling_ni` db_won 이 `ifrs-full_ProfitLossAttributableToOwnersOfParent`(진짜 순이익 귀속)가 아니라 `ifrs-full_ComprehensiveIncomeAttributableToOwnersOfParent`(총포괄손익 귀속) 값과 정확히 일치 — face_audit.py 는 이미 정답을 찾아 fail_a 로 올바르게 flag 중이었다(R47 무관, §2-G 참고). `account_mapper.py` 의 "맨몸 지배지분 라벨 가드"(2026-08-22 도입)가 오늘 코드로 이 두 문서의 오염 라벨을 이미 정확히 차단하는 것으로 확인됨(`get_mapper().map("지배기업소유주", fs_section="is")` → `unknown`) — 두 필링(rcept 2025-05-15/2026-03-18, 둘 다 가드 도입일 이전) 모두 **재표준화 미반영 stale 값일 가능성이 높음**(std_financials_v3 에 표준화 시각 컬럼이 없어 직접증명은 못함). **다음 단계**: 이 2건만 `build_std_v3.py` 재빌드 후 db_won 이 report_won 과 일치하는지 확인(DB 쓰기, 사용자 확인 후 진행) — 재빌드로도 안 고쳐지면 가드가 못 잡는 제3의 경로가 있다는 뜻이므로 새 R번호로 분리 필요 |
