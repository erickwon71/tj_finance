...
**측정**: DB 재구성 스캔(`scripts/scan_r162d_dual_evidence_db.py`, 근사치 —
DB 행에 다시 돌리는 스캔은 적재 시점 행 집합과 달라 멱등이 아니다, 규모는
재추출 전후 비교로만 확정) — 최종 설계로 candidates(증거 무시) 8,753필링/32,121셀,
이중증거+게이트 통과 **7,030필링/18,068셀**. 표본 원문대조 8건 전부 기대와 일치.

**재적재**: `docs/qa/r162d_row_identity_handoff_2026-09-25.md`(2026-09-22 최초 연구
문서)·`docs/qa/r162d_dual_evidence_candidates_2026-09-25.jsonl`(후보 목록) 참고.
`vq.py batch add-targets` 로 대상 필링 등록 후 `vq.py batch reload` — 재추출은 SD
(`/Volumes/dart_data/raw_report`)+NAS 분할 샤딩으로 돌린다(NAS 단독은 SMB
병목으로 3.5시간에 13%만 진행됐던 전례, 메모리 `feedback-bulk-read-use-sdcard`).
...
| **R162-d — SCE 행 내부 열 항등식(구성요소 합 = 그룹 합계)을 부호 앵커로, 이중증거 셀만** | **✅설계 확정·구현·테스트 완료 · 백필 진행 2026-09-25**. 2026-09-22 최초 연구(3,903셀/2,398필링)는 백필 보류됐으나, 표본 원문대조 8건(전부 참으로 확인, 손익결과 항목의 교차-basis 오탐 우려는 방어적 게이트로 유지)을 거쳐 최종 설계로 확정 — 자본거래/이월잔액이 아닌 행은 같은-basis 증거만 인정. 이중증거+게이트 통과 **7,030필링/18,068셀**. 상세 = R162-d |
