...
### 3.3 드리프트 근본 원인은 미규명 상태로 남는다

어서션은 **탐지·차단**이지 치료가 아니다. 누가/무엇이 심링크를 바꾸는지는 아직 모른다(후보: 자동 마운트 순서, 과거 폴백 스크립트 잔재, Time Machine). G2 실패 로그에 타임스탬프가 남으면 재발 시점을 특정할 수 있고, 그때 원인을 좁힌다. 이 계획은 **재발해도 데이터가 안 깨지는 상태**까지만 보장한다.
