"""Ad-hoc: after the T4 M3 code fix, re-run the REAL production
declaration_text()/inherited_declaration_text() (not the probe's copy) against the 30
M3-flagged tables to confirm they now resolve. Empirical check, not hand-tracing."""
...
from fin2.extract.statement_titles import SECTION_CODE_OF            # noqa: E402
from fin2.extract.text import (                                      # noqa: E402
    _detect_fin_type, _detect_body_statement_tables,
    declaration_text, inherited_declaration_text)
...
def main():
    ...
    found, still_none, no_table = 0, 0, 0
    ...
    with get_session() as s:
        for r in m3:
            ...
            if path is None:
                print(f"{r['rcept_no']} -- file not found")
                continue
            ...
            if tbl is None:
                no_table += 1
                print(f"{r['rcept_no']} {r['corp_code']} {r['statement']}/{r['basis']} -- table not found")
                continue
            decl = declaration_text(tbl) or inherited_declaration_text(tbl)
            if decl is not None:
                found += 1
                print(f"OK  {r['rcept_no']} {r['corp_code']} {r['statement']}/{r['basis']}  -> {decl[:70]!r}")
            else:
                still_none += 1
                print(f"!!  {r['rcept_no']} {r['corp_code']} {r['statement']}/{r['basis']}  -> still None")

    print(f"\nfound={found}  still_none={still_none}  no_table={no_table}  total={len(m3)}")
