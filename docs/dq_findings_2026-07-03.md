...
## 4. 연결 자산 < 별도 자산 (2,761) — 대부분 정상, 22행만 진짜 오류

sep/con 자산 비율 버킷 분석:
- **≥1e5(≈×10^6) 1행** + **100~1e5 21행** = **22행이 진짜 magnitude 오류**(별도가 연결의 100배+ 는 불가).
- 2~100× 28행(모호), **<2× 2,711행**(비율 1.0~1.9 = 대부분 정상 — 내부거래 제거·모회사 투자평가 차이).
→ **일괄수정 안 함**: 22행도 어느 basis 가 틀렸는지(별도 인플레 vs 연결 과소)는 DART 대조로 건별
판정해야 안전. WARN 어서션(consolidated_lt_separate_assets)으로 상시 추적. 후속: 22행 건별 교차검증.

## 5. op==ni 잔여(interim) — 확장 수정

FY/Q1 수정 후 잔여는 interim(H1/Q3) + 대체후보 없는 케이스. `_collect` 교정을 interim(누적셀 후보
한정)까지 확장 → 재표준화로 H1/Q3 도 대폭 감소(WARN 어서션으로 잔여 추적).
