...
**★전사 스코프는 아직 미완결.** SQL 후보(EPS 원선언 스타일 + 같은 필링 IS 본문에
`중단영업이익` 이 있음)로 좁혀도 **5,904건/708개사**가 남는다 — 두산에너빌리티는 이
후보군의 1개사일 뿐이다. 다음 세션에서 전체 재추출 스캔(스캔 스크립트는
`docs/qa/eps_continuing_ops_subline_gap_r147_2026-09-20.md` 에 재현 방법 기록)을
마저 돌리고, 실제 결측이 확인된 회사만 이 백필 스크립트에 `--rcept-list` 로 넘겨
재적재할 것. 이 결함은 R146(D&A 이중계상)처럼 재발 여지가 없는 **완결된 근본수정**
이지만, **소급 백필은 두산에너빌리티 1개사만** 끝난 상태다.
