...
# ── 로마숫자 접두어(유니코드 한 글자=Ⅲ 처럼 여러 자리를 나타냄) → ASCII 다중문자 치환표.
# normalize_account_name() 의 혼합표기('XⅢ.' 처럼 ASCII 와 유니코드가 섞인 접두어) 정규화에 씀
# — 대문자/소문자 유니코드 로마숫자 모두 대문자 ASCII 로 치환(제거만 할 것이라 대소문자 무의미).
_ROMAN_UNICODE_TO_ASCII: dict[str, str] = {
    'Ⅰ': 'I', 'Ⅱ': 'II', 'Ⅲ': 'III', 'Ⅳ': 'IV', 'Ⅴ': 'V',
    'Ⅵ': 'VI', 'Ⅶ': 'VII', 'Ⅷ': 'VIII', 'Ⅸ': 'IX', 'Ⅹ': 'X',
    'ⅰ': 'I', 'ⅱ': 'II', 'ⅲ': 'III', 'ⅳ': 'IV', 'ⅴ': 'V',
    'ⅵ': 'VI', 'ⅶ': 'VII', 'ⅷ': 'VIII', 'ⅸ': 'IX', 'ⅹ': 'X',
}
_LEADING_ROMANISH_RE = re.compile(r'^[A-Za-zⅠ-Ⅹⅰ-ⅹ]+')
...
#   ★R179(2026-09-26) — 음수 표기 자체가 '원'까지 감싼 '(89원)' 도 커버한다(뒤에 ')' 가
#   하나 더 와도 매치). 안 하면 '89원'/'(2,564)원'(원이 괄호 밖)은 잡히는데 '원'이
#   괄호 **안**에서 끝나는 경우만 못 잡아 EPS 손실 행(음수)이 통째로 결측된다(SK스퀘어
#   2021FY 사업보고서 [별도] IS '기본 및 희석주당순손실 (89원)' 실측, 이슈#17).
...
def normalize_account_name(raw: str) -> str:
    ...
    # ★혼합 로마숫자 접두어 정규화(2026-09-02, R62): 유니코드 로마숫자 한 글자(Ⅲ='III')는
    # 여러 자리를 나타내 ASCII 문법 정규식과 안 섞인다 — 'XⅢ.'(ASCII 'X' + 유니코드 'Ⅲ')처럼
    # DART 필자가 섞어 쓰면 옛 두 정규식이 각자 반쪽만 지워 'Ⅲ. 총당기순이익' 같은 잔재를
    # 남겼다(뒷 alias 매칭이 이 잔재 형태와 안 맞아 net_income 결측으로 이어짐, 실측
    # 현대차 등). 앞쪽 로마숫자류 접두어 구간의 유니코드 문자만 ASCII 다중문자로 치환한 뒤
    # 기존 ASCII 문법 정규식 하나로 통일 처리 — 과매칭 방지 문법(유효한 로마숫자만 인식)은
    # 그대로 재사용되므로 일반 라벨(비-로마숫자 접두어)의 동작은 변하지 않는다.
    s = _LEADING_ROMANISH_RE.sub(
        lambda m: "".join(_ROMAN_UNICODE_TO_ASCII.get(ch, ch) for ch in m.group(0)), s,
    )
    # 로마숫자 접두어 제거 (ASCII로 통일된 형태: I, II, III, IV ... XV, XVI 등)
    ...
    # ★ 와/과/및 결합표기 포함(2026-09-02): 복수 주석번호를 콤마 대신 한글 접속사로
    #    묶는 표기('(주석13과 14)', '(주석1과15)', '(주석5,7과29)' 등)가 구 정규식
    #    `[\d,\s]*` 로는 안 잡혔다 — 숫자 뒤 '과'/'와'/'및'에서 매치가 끊겨 괄호가
    #    안 지워지고 남았다. 실측: DB 전수 스캔 2,260종 라벨/6,452행이 이 갭으로
    #    account_mapper 매핑실패(unknown)했다(자본금·매출액·유형자산·매입채무 등
    #    핵심 계정 포함, KG스틸 revenue 복구실패가 발단). 숫자 목록 안의 구분자
    #    문자셋에 와/과/및 을 추가해 통일 처리.
    s = re.sub(r'\(주석?\s*\d[\d,\s와과및]*\)', '', s)
    s = re.sub(r'\(Note\s*\d+\)', '', s, flags=re.IGNORECASE)
    # 주석 참조 — 후방 제거: "계정과목 (주5)" 형태
    s = re.sub(r'\s*\(주석?\s*\d[\d,\s와과및]*\)\s*$', '', s)
