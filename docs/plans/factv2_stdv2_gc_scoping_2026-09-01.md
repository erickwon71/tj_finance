...
2. pre-1999 249행 정책 결정 — **범위 밖, 별도 트랙**(§6 후보 D). ★**결정 완료
   (2026-09-08)**: 유니버스 제외로 확정 — v3 범위 확대(백필) 안 함, 영구 갭으로 수용.
   근거: 1999년 이전은 DART 전자공시 시행 전이라 소스 신뢰도가 낮고, 249행이라는
   규모 자체가 작아 백필 트랙을 새로 여는 비용 대비 실익이 낮다고 판단. 코드 작업
   없음, 문서화로 종결.
