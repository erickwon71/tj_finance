...
#
# ★ 2026-09-07(account_mapper 전체어휘 스캔, "취득/처분" 반의어쌍 갭): CF/NOTE 섹션에
# "취득"(자산증가·현금유출) canonical 만 있고 그 반의어 "처분"(자산감소·현금유입) 전용
# canonical 이 아예 없는 케이스 6종. 정확한 alias 를 추가하는 대신(=취득 코드에 처분
# 라벨을 등록하면 부호가 반대인데도 같은 계정으로 합산됨) 여기 등록해 **fuzzy 로
# 취득쪽에 오매핑되는 걸 막는다** — 무매핑(unknown.*)으로 남는 게 부호오염보다 낫다
# (결측이 오염보다 낫다 원칙). 전용 처분(inflow) canonical 신설은 스키마 확장이라 더 큰
# 결정 필요 — 이번엔 방어만(§8-12 후속, 사용자 선택: 방어 우선).
_FUZZY_BLOCK: set[str] = {
    "종속기업의처분", "관계기업의처분", "공동기업투자의처분", "공동기업의처분",
    "투자부동산의취득", "기계장치의취득", "차량운반구의취득", "산업재산권의처분",
    "자기주식의처분", "자기주식처분금액",
}
