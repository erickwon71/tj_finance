...
⚠ 알려진 한계 — 증감표(rollforward)
    유형자산/무형자산 증감표는 컬럼이 **자산 분류**(토지·건물·기계…)라 기간축이 아니다.
    형제표로 묶이지 않으면 MULTICOL 로 오인해 col_index 를 기간으로 읽는다.
    실측 예(00103592 FY2024): '12. 유형자산' 감가상각비 행의 col_index 0 이 0 이고 실제 값은
    col 4·5 에 있었다. 컬럼 헤더(col_label)가 계층2 에 전사되지 않아(전량 NULL) DB 만으로는
    구분할 방법이 없다.
    완화책: DA_SOURCE_PRIORITY 에서 증감표 계열(PPE/무형/투자부동산)을 성격별·현금흐름표보다
    **뒤에** 두고, 값이 0 이면 다음 소스로 넘어간다(실제로 위 사례는 이 경로로 회피됐다).
    근본 해결은 계층2 가 col_label 을 전사해야 가능(주석 재파싱 필요).
