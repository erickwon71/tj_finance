...
### ✅ attempt-tracking 추가 (2026-07-17 해결)
기존엔 Phase 4 가 실패한 복원 시도를 기록하지 않아, D&A 를 끝내 미공시하는 영구-NULL 기업이 매일
재시도되고 `잔여`가 0 에 도달하지 못해 **잡이 자기 해제되지 않을 수 있었다**. Phase 3 와 동일한
상태파일 방식(`~/.tj_finance/gap_fill_phase4_state.json`)을 추가해 **성공·실패 무관 1회 시도한
기업은 기록·스킵**하도록 수정했다(`run_phase4`). 이제 미시도 기업이 모두 소진되면(≈500사/밤 ×
2,055사 ÷ = 4~5밤) `잔여 미시도`가 0 이 되어 Phase 2·3 와 함께 잡이 정상 자기 해제된다.
검증: 배치 2사 실행 시 복원 1사·불가 1사 **둘 다 attempted 기록**, 잔여 2055→2053 감소 확인.
(신규 보고서 분은 collect_new 파이프라인이 별도 커버.)
