...
- [x] **4-5** `[R]` 신규 fail_a(209건) 트리아지 — 완료. `track` 분포 A 158/B 51,
      ...
      (수익체감 판단, 사용자 방향 확인됨).
- [ ] **4-6** `[W]` 데일리 알림 임계 재설정 — **미착수**. `collect_new.py:234, 765`.
      4-3/4-5 결과로 나온 정상 배경 수준(baseline noise, 이제 fail_a 209/152,017≈0.14%)을
      반영. 무조건 `vd>0` 발화가 부적절하다고 판명되면 임계 도입 또는
      **트랙별 분리 집계**. 다음 세션 착수 대상.

> **게이트 4**: 4-4가 **전이 0건**일 것(절대 조건) — **충족(대체 증거로)**. 4-5에서
> 클러스터가 전부 원인 특정될 것 — **충족**(상위 2개 클러스터 원인 가설 수립, 나머지는
> 소액 분산이라 개별 특정 생략). **Phase 4 사실상 완료, 4-6만 잔여.**
