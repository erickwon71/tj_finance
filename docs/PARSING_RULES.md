...
★**앵커 부족이 아니다** — 방향은 알지만 `기초 + Σ변동 = 기말` 이 안 닫힌다. 유력한
원인은 **소계 행 이중계상**(부모 '총포괄손익' 행이 값을 갖고 자식 행도 값을 가지면
Σ변동이 두 번 센다 — 효성중공업 연결 `지배기업 소유주지분 합계` 열에서 실제로 관측)과
`회계정책변경의 효과`·`연결범위의 변동` 같은 비정형 행이다. `node_role` 로 소계를
빼면 더 닿을 수 있다 — **미착수, 스코프 확장이라 사용자 확인 필요.**
그때까지 이 잔여는 R6 대로 **손대지 않는다**(부호가 틀린 채 남아 있지만 증명이 없다).
