...
| **R47-b — 앵커 정규식(`_NI_TOTAL_RE`) 불일치(34%, 24/70건)** | **미조치** — TE판·TD판이 공유하는 정규식 `^당?(기|분기|반기)순(이익|손익)`이 실제 라벨 변형을 다수 못 잡음: ①로마숫자/번호 접두(`"XIII.당기순이익(손실)"`, `"(1)지배기업소유주지분"`), ②`"연결"` 등 접두(`"연결당기의순손실"`), ③`"순손실"`이 `이익|손익` 대안에 없어 원천 미매치(서희건설 00219848 등). TE/TD 양쪽 함수 공통 결함이라 R47-a와 무관하게 둘 다 막힘. 설계 미착수 — 접두 스트리핑 + `순(이익|손익|손실)` 확장 방향, 광범위 코퍼스 대조로 오매치(예: `법인세비용차감전순이익`) 검증 필요 |
| **R47-c — `_detect_body_statement_tables()` 스코프 미스(27%, 19/70건)** | **미조치** — 정답이 있는 표가 IS 본문표 반환집합에 아예 없음. 표본 1건(00138729 아세아제지 2022FY) 확인: 정답이 "1. 요약재무정보" 하이라이트 표에 있고, 실제 본문 재무제표에 같은 값이 다른 라벨로 있는지는 미확인 — 19건 전체 특성화 미착수 |
| **R47-d — ACONTEXT 없는 XBRL 방언(6%, 4/70건)** | **미조치** — ACODE 는 있으나 별도 ACONTEXT 속성이 없고 컨텍스트/축/단위 정보가 ACODE 문자열 자체에 파이프(`concept\|context_axis\|decimals\|unit\|`)로 붙어 있는 필러 방언(01137383 카카오게임즈 등, 문서 전체 TE[@ACODE] 4,016개 전수 확인 결과 별도 ACONTEXT 속성 0개). `parse_acontext()`(`fin2/extract/acontext.py`)가 이 형식을 못 읽어 구조함수가 침묵. 파싱 확장 방향 미설계 |
