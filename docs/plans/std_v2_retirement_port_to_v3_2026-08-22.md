...
> **★2026-08-30 정정 — 이 결론은 설계상으론 맞지만 지금은 사실이 아니다.**
> `valuation_daily_v3_migration_plan_2026-08-30.md` §Phase 0-1 실측: `note_da.py`가 주제
> 판정에 쓰는 `report_tables.section_path`(F3 리팩터, 2026-07-31)가 **F3 도입 이전에
> 추출된 기존 corpus에 소급 백필된 적이 없다**(FY2024+ note 보유 rcept 26,518건 중
> `report_tables` 매칭 19건뿐, 0.07%). 그래서 `note_da.py`가 지금은 거의 항상 빈손을
> 반환하고, v2가 값을 갖는 건 v3보다 넓은 소스를 봐서가 아니라 `cf_da_sync`가
> `note_lines`를 안 거치고 **원문 XML을 직접 재파싱**(`recover_cf_da()`)하기 때문이다.
> **코드 버그가 아니라 소급 백필 누락**(런북 "② 소급 백필은 자동이 아니다") — `report_tables`
> 백필이 끝나면 위 "상위집합" 결론이 실제로 성립할 것으로 예상된다. 백필 전까지는
> 이 §3.10의 "이식할 로직 없음" 판단에 의존해 v2 잔여 경로를 끄면 안 된다.
>
> **★2026-08-30 백필 완료 후 확인 — 예상대로 원래 결론이 맞았다.**
> `report_tables_note_backfill_plan_2026-08-30.md` Phase 1(주석 전량 재적재 + std_v3
> 전수 재빌드) 실행 후 재대조: `report_tables(note)` 커버리지 0.07%→사실상 완전,
> 표본 20건 중 11건이 "v2엔 없던 값을 v3가 새로 확보"(v2의 cf_da_sync는 fy≥2024만
> 패치해 그 이전은 원래 없었는데 v3의 note_da.py는 연도 무관하게 채움). Gate B
> 표본(40개사, seed=42) 재감사도 v2 fail_a=170 / v3 fail_a=169로 사실상 동일 —
> 등급 전이 0건. **이제 "이식할 로직 없음"이 실제로 성립한다** — 다만 §D3
> (`_sync_cf_da`의 `standardize_corp(v2)` 재전파 제거)는 여전히 `valuation_daily`
> 등 v2 잔여 소비자 이식이 끝난 뒤에 하는 게 맞다(위 결론과 별개 사안, D1-b 순서 유지).
