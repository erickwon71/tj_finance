...
## 6. 미결정 — 사용자 결정 필요 (rev2 갱신)

1. **알림 수단** — 현재 프로젝트에 알림 코드가 전무(§5-B). 후보:
   (a) macOS 알림 팝업(osascript), (b) 수집 실행 끝에 요약 줄 출력, (c) 둘 다,
   (d) 알림 없이 JSON 파일만(rev1 원안).
2. **T1-3 `_FX_PRESENTATION_CURRENCY_KEYS` 를 이번 범위에 넣을지** — 항등식이 없어
   탐지 규칙부터 새로 정의해야 함(§5-D). 나머지 T1 2종 + T2 만 먼저 하고 이건 분리하는
   선택지 있음.
3. **T0 축 B(신규 회사가 blanket override 대상이 되는 경우) 를 이번 범위에 넣을지** —
   rev2 에서 새로 드러난 항목. 3종 다 수동 생성이라 §5-D 와 같은 신규 작성 부담.
4. **리뷰 큐 형식** — JSON 파일로 충분한지, 아니면 DB 테이블(`curated_key_candidates`)로
   둬서 이력 추적을 할지.
5. **2026 반기 백로그(약 2,200건) 적재 시점** — ④ 배선 후 적재하면 그 실행에서 탐지가
   같이 돌아 재발을 그 자리에서 잡는다. 먼저 적재하면 밀린 데이터는 빨리 채워지지만
   재발 검출은 나중 전수감사로 미뤄진다.
