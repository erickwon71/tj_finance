...
**후속(2026-09-05, 같은 날 이어서)**: (A-3) 전용 설계 완료 —
`docs/plans/category_c_a3_squished_cell_bs_total_assets_design_2026-09-05.md`.
전체 라인아이템 재구성은 여전히 위험하다고 확정됐지만, "부채와자본총계가
항상 BS의 마지막 줄"이라는 서식 관행 + 회계항등식(자산=부채+자본)을 이용해
**`total_assets` 하나만은 위치와 무관하게 안전하게 복구 가능**함을 실측으로
검증(3개사 원문대조 + 인접기간 성장궤적 일치 확인, 무작위 80건 게이트
재현율 100%). 상세는 그 문서 참고.

**권고(구)**: (A-3)은 **별도 트랙**으로 분리 — 정량화(§3) 먼저, 그 다음 전용 설계
