...
**스코프**: 연결 CF 는 있는데 별도 CF 가 0행인 필링은 2015+ **10건/9개사**뿐이고, 그중
SCE 가 부풀어 오분류 신호가 강한 건은 **2건**이었다. 두산밥캣 `20170331005642` 은 열어
보니 **표 분류가 전부 정상**이었고(SCE 201행도 50행 × 4열이라 정상) 별도 CF 0행은
**다른 원인**이다 — 미조사. 이 메커니즘은 삼성화재 계열로 좁게 본다.
