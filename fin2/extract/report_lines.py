...
_MANUAL_UNIT_OVERRIDE_MULTIPLIER_RCEPTS: dict[str, int] = {
    ...
    # ★2026-09-16(같은 날 후속, BS 임계값 +3 확장 재검토 — 사용자 지시 "상폐21개사
    # 삭제하고 이어서 +3씩 올려서 확인") 1건 추가.
    "20230323001157": 1,  # 소프트센 2022FY — 재고자산 원문 그대로 4,334,282,109(43억원
                          # 규모가 소형사 재고자산으로 타당, 백만원 그대로면 4.3경원
                          # 불가능값). 자산총계/부채총계/자본총계 등 총계행이 전부
                          # _AMOUNT_SANE_MAX 초과로 결측된 채(BS_consolidated 22행,
                          # 세부항목만 잔존) 임계값 확장 스캔에 걸려 발견.
}

# R169 — the same defect, confirmed per (rcept, section) by
# `scripts/unit_self_contradiction_scan.py` instead of by hand: the section's raw amounts
# read at multiplier k match the same company's other filings exactly (prior-period
# comparatives), and no other k does. The data file is generated by that scan (--apply);
# a section absent from it keeps its declared unit (R6).
_PROVED_UNIT_OVERRIDES_PATH = (
    Path(__file__).resolve().parent / "data" / "unit_self_contradiction_overrides.json"
)


def _load_proved_unit_overrides() -> dict[str, dict[str, int]]:
    # Fail loudly if the file is missing — a silent empty dict would make reloads
    # quietly re-store the wrong unit.
    with open(_PROVED_UNIT_OVERRIDES_PATH, encoding="utf-8") as f:
        return {rc: {code: int(m) for code, m in secs.items()} for rc, secs in json.load(f).items()}


_PROVED_UNIT_OVERRIDES: dict[str, dict[str, int]] = _load_proved_unit_overrides()


def _unit_override(rcept_no: str, section_code: str) -> tuple[int, str, str] | None:
    """(multiplier, unit_source, decl_raw) when the declared unit is known to be wrong."""
    if rcept_no in _MANUAL_UNIT_OVERRIDE_MULTIPLIER_RCEPTS:
        # unit_source 컬럼은 String(14) — "manual_unit"(11자)로 짧게 표기.
        return (_MANUAL_UNIT_OVERRIDE_MULTIPLIER_RCEPTS[rcept_no], "manual_unit",
                "R132 자기모순 단위선언 수기교정")
    m = _PROVED_UNIT_OVERRIDES.get(rcept_no, {}).get(section_code)
    if m is not None:
        return m, "proved_unit", "R169 자기모순 단위선언 — 타 필링 동일값 대조로 확정"
    return None
...
def _emit_section_lines(
    ...
    *,
    ...
    for table in data_tables:
        ...
        # R169 — 스캔으로 확정된 (rcept, section) 도 같은 자리에서 적용한다.
        override = _unit_override(rcept_no, section_code)
        if override is not None:
            unit, unit_source, decl_raw = override
        if unit is None:
...
def _emit_sce_lines(
    ...
    **자본잉여금 열이 '전기 데이터'로 둔갑**한다. 주석 슬라이스와 같은 규약을 쓴다:
    ...
    for table in tables:
        ...
        # R169 — R132/R169 overrides were applied to BS/IS/CF only, so SCE in those filings
        # kept the ×10⁶ value. Same override, same place in the unit resolution order.
        override = _unit_override(rcept_no, section_code)
        if override is not None:
            unit, fx_source, fx_decl = override
        if unit is None:
...
def store_report_lines(session, rcept_no: str, lines: list[ReportLineRow], *,
    ...
    ★원문대조 완료 보호(R139, 2026-09-18) → **2026-09-24 사용자 결정으로 폐지·대체.**
    예전에는 `layer2_review_queue.status='pass'` 인 rcept 를 여기서 하드 거부했다. 이제
    검증 상태는 `verification` 스키마가 들고, report_lines 에 걸린 적재 트리거
    (`fin2/verification/schema.sql`::trg_finalize_load)가 **내용이 실제로 바뀐 경우에만**
    그 필링을 passed → pending 으로 되돌려 재검증 큐에 넣는다(바뀐 scope 목록과 함께).
    그래서 "passed" 는 언제나 현재 적재본 기준이고, 거부 대신 재검증으로 안전을 보장한다.
    검증 중(lease) 인 슬롯은 같은 트리거가 수정 워크트리의 적재를 막는다.
    `overwrite_reviewed` 인자는 기존 호출부 호환용으로 받기만 하고 무시한다.
    설계: docs/plans/verification_schema_two_worktree_design_2026-09-24.md §4."""
    from sqlalchemy import delete, insert, select
    from collector.models import ReportLine

    del overwrite_reviewed  # R139 retired 2026-09-24, see docstring
