...
_SOURCE_TYPO_CELL_FIXES = {
    ...
    # 삼성E&A 00126308 20240313000522(2023 사업보고서) + 기재정정 20240314001768
    # (byte-identical 표) 별도 자본변동표 2023 '당기순이익(손실)' 이익잉여금 칸 —
    # 마침표 오타에 **숫자 하나까지** 틀렸다('278.632' vs 정답 '277,632'). 그래서
    # R158(행 안 정수 짝)도 dot→comma 가설 항등식도 안 닫혀 R160 결측으로 남았다.
    # 정정값은 원문 인쇄값으로 추측 없이 확정된다(검증 이슈 #18060/#18208, 2026-09-26):
    #   → 같은 행 항등식: 자본금 0 + X + 기타자본구성요소 0 = 자본 합계 633,492,277,632
    #   → 같은 필링 별도 손익계산서 당기순이익 633,492,277,632 (원문에 정수 5회 인쇄)
    ("20240313000522", "633,492,278.632"): "633,492,277,632",
    ("20240314001768", "633,492,278.632"): "633,492,277,632",
