...
**미해결 카운터 사례 — 특수건설 20151116001903** — 같은 "재무제표_직접작성" 서식이지만
...
근거를 못 찾는다. **이 규칙 밖의 별도 결함** — 부록 C 미결 항목 참고.
...
| **특수건설 20151116001903 — 제목+데이터 병합 표** | **미조치**. BS·IS 가 한 TABLE 에 제목·기간·회사명·단위·데이터가 전부 들어있어 `title_text_owned`(직전 형제 탐색 전제)가 제목을 못 찾아 분류 자체가 실패한다. R4-1(문서 기본 단위)로도 못 고친다 — 이 문서는 요약재무정보·회계정책 주석 둘 다 없다(분기보고서 간이서식). 별도 분류기 확장 필요 |
