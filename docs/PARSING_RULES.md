...
**스코프**: Q1 필링 중 `[3개월|누적]` 2단 헤더 표 전부. 2015+ Q1 손익계산서 적재
26,339건 중 EPS 0건인 907건(505개사)이 후보이고, **재추출 dry-run 실측으로 120건 중
25건(20.8%)이 실제 복원**된다(적재되는 col_index=0 행 기준 82행). 나머지는 원문에
당기 EPS 가 없는 정상 건이다. 백필 대상은 `scripts/backfill_r154_q1_eps.py` 의
재추출 결과로 확정한다(당기 행이 하나라도 나오는 필링만).
