# 계획 — fact_v2/std_financials_v2 DROP 이후 보강 백로그 (2026-09-01)

> **2026-09-02 진행 기록**: 사용자 지시로 우선순위 1~5(§2 표) 착수, 6(v2 UNION 정책
> 재검토)은 제외. §1/§4는 구현·검증 완료(커밋 전 워킹트리). §2/§3은 원문대조 기반
> 원인규명 완료(코드 변경 없음). §5는 조사 중 원 스코핑이 틀렸음을 발견 — 구현 전
> 사용자 결정 필요(아래 각 섹션 "2026-09-02 진행" 참고).

> 상태: **리스트업만 — 실행 계획 아님.** [정책](../../CLAUDE.md) 상 계획 작성 후 자동 실행
> 금지, 항목 선택 및 착수 승인은 별도 요청 대기. 배경 = 계층3 GC(`std_financials_v2` DROP,
> 커밋 `510095a`) + 계층2 GC(`fact_v2` DROP, 커밋 `31bc4e4`+`f76f4f7`) 완료 이후 두 세션에서
> 발견된 "DROP으로 기능이 조용히 빠지거나 미완으로 남은" 항목을 한곳에 모은 것.

---

## 0. 한 줄 요약

두 GC 트랙(계층2 `fact_v2` 55GB, 계층3 `std_financials_v2` 386MB) 자체는 **완전 종료**됐고
Gate B 전수재감사·pytest·dq_assertions로 크래시·회귀는 확인 안 됐다. 다만 실행 과정에서
"DROP 전에는 v2가 대신 채우고 있던 기능/감지"가 **몇 곳 조용히 비게** 됐고, 그중 일부는
이미 문서화만 되고 착수는 안 된 상태다. 아래 7개 항목을 발견 경위·현재 영향·조치 방향
순으로 정리한다.

---

## 1. dq_assertions.py WARN 어서션 2건 영구 SKIP — 회귀탐지 기능 상실 ★최우선 후보

- `std_v2_controlling_ni_exceeds_net` — controlling_ni 총포괄손익 오염 재발 감지
- `fact_v2_q1_duration_col0_eq_col1` — DEF-4 Q1 전기컬럼 중복추출 감지

`fact_v2` DROP으로 두 어서션이 대상 테이블을 잃어 `scripts/dq_assertions.py`에서 상시
SKIP(방어적, ERROR 아님)이다. dead-code 정리가 아니라 **실제 회귀 탐지 기능 상실** —
두 어서션이 잡던 버그 클래스(controlling_ni 오염 R25/R26/R43 계열, EPS·주식수 Q1 중복추출)는
과거 여러 트랙에서 실제로 재발한 전례가 있다. `report_lines`/`note_lines` 기반 재구현이
필요(단순 포팅 불가 — 원래 acode 기반 조건이라 재설계 필요).

**영향도**: 높음(회귀를 놓칠 수 있는 감시 공백). **난이도**: 중간(재설계 필요하나 표면적은
좁음, 어서션 2개).

> **2026-09-02 진행 — 완료.** `std_v2_controlling_ni_exceeds_net`: `fact_v2` NOT EXISTS
> 절을 `extended_facts_v3`(is.noncontrolling_ni, §4-2 재설계로 이미 존재하는 단일 확정값)
> 로 재소싱해 복구. 실측 357건/223개사(과거 fact_v2 기준 수치와 모집단이 달라 직접비교
> 불가). `fact_v2_q1_duration_col0_eq_col1`: 재구현 **불가로 결론**하고 폐기 — `report_
> lines.py:1199 _is_loadable()`가 BS/IS/CF는 당기(col_index=0)만 적재한다는 2026-07-30
> 결정 때문에 "같은 문서 col0 vs col1 직접비교"라는 이 어서션의 판정 방식 자체를 재현할
> 데이터가 DB에 없다(SCE/note 는 전 컬럼 적재되지만 BS/IS/CF엔 해당 안 됨, 실측 확인:
> `report_lines` col_index 분포 자체는 전 statement 합산치라 착시 가능 — BS/IS/CF만 보면
> col_index=1 행 0건). 대체 신호로 이미 살아있는 `calendar_adjacent_year_cq1_identical`을
> 공식 후계로 지정(코드 주석 갱신). `scripts/dq_assertions.py` 전체 실행 exit 확인,
> 크래시 없음. 코드는 `scripts/dq_assertions.py` 워킹트리 변경(커밋 안 함).

## 2. `standard_financials` 뷰 v2 UNION 브랜치 제거 — 12,149건(73%) 영구 결측

std_v2 DROP 시 사용자가 감수하기로 결정한 트레이드오프. `report_lines`의 "당기(col_index=0)열만
적재" 정책(2026-07-30 결정)의 직접 결과로, 그 연도의 단독 필링이 없는 회사(IPO 시점이
늦어 이전 연도는 비교열로만 존재)의 과거 시계열이 통합 뷰에서 사라졌다. 표본
(`01593668` FY2021)으로 원인 확정됨.

**되돌리려면**: 2026-07-30 정책 자체를 재검토(비교열도 `report_lines`에 저장)하는 새 설계
작업 필요 — `note_periods`급 규모. **영향도**: 중간(오래된 과거 데이터 깊이 문제, 최신
데이터엔 무영향). **난이도**: 높음(정책 재검토 + 전사 재백필).

## 3. Category C 4,366건 — 표(재)백필 실행했으나 결측 순감소 0건, 진짜 원인은 파서 구조적 한계

애초 "도구(`sync_layer2_lines`)만 돌리면 되는 백로그"로 스코핑했으나 실제 실행 결과
결측이 전혀 안 줄었다. 재조사 결과 처리 필링의 99.8%가 `extract_report_lines()`의 표
탐지 로직에서 "본문 섹션 없음"으로 끝남 — 표본(`20000110000001`, 한솔전자 FY1999 H1,
22KB 구형 `<DOCUMENT>` 포맷)으로 이 시대 포맷의 표 구조를 현재 파서가 아예 인식 못 함을
확인. **기존 "pre-2015 2차패스"(`docs/plans/pre2015_layer2_backfill_plan_2026-08-10.md`,
2026-08-11 완료 표기)·"PDF-only 3차패스" 트랙과 대상이 겹치는지 아직 미확인** — 새 R-트랙
착수 전 반드시 먼저 확인해야 함(pre-2015 2차패스가 "완료"로 기록돼 있는데도 이 결측이
남아 있다는 건 그 트랙이 커버 못 한 별도 서브포맷이거나, 완료 판정 자체가 부분적이었을
가능성).

**영향도**: 중간(922개사·4,799rcept, 전부 fy≤2017 — 오래된 데이터). **난이도**: 높음
(신규 파서 확장, 조사 먼저 필요).

> **2026-09-02 진행 — 조사 완료(코드 변경 없음).** 두 표본을 실제로 `extract_report_
> lines()`에 통과시켜 비교: 한솔테크닉스 1999H1(`20000110000001`, 최초 트리아지 표본)은
> **여전히 "본문 섹션 없음"으로 실패**(0행) — pre-2015 2차패스 "완료" 표기와 무관하게
> 이 최초기(1999~2001 추정) 서브포맷은 지금도 못 읽는다. 반면 대한광통신 2006 Q3
> (`20061114000051`, 같은 `<DOCUMENT>` 루트 포맷)는 **정상 추출됨**(320행, BS/IS/CF
> 전부). **결론: Category C는 "pre-2015 `<DOCUMENT>` 포맷 전체"가 아니라 그중에서도
> 더 이른 시기의 특정 서브포맷만의 문제로 범위가 좁혀진다** — 원 스코핑("99.8% 본문
> 섹션 없음")은 표본 편향(오래된 fy 표본만 뽑았을 가능성)일 수 있어, 착수 전 fy별
> 성공/실패율을 다시 실측해야 정확한 규모가 나온다(이번엔 표본 2건만 확인, 전수 재실측
> 미실행). §5(아래)와 부분적으로 같은 뿌리(대한광통신 표본이 실제로 §5 위반 150건 중
> 하나였음)일 가능성이 있어 함께 스코핑하는 편이 효율적.

> **2026-09-03 진행 — fy별 전수 재실측 완료, 2026-09-02의 "범위가 좁혀진다" 결론을
> 정정(코드 변경 없음).** `std_financials_v2`가 이미 DROP돼 원 Category C 정의 SQL(§5-1)을
> 그대로 재실행할 수 없어, 2026-09-01 백필 실행 시 저장해둔 corp 목록(922개사,
> `/tmp/backfill_c_corps_2026-09-01.txt`)을 시드로 `download_tasks`(xml completed) ×
> `filings` 조인 후 `report_lines` 미존재 rcept를 fy1999~2017 전 구간에서 뽑아
> **12,960건 전수**에 대해 실제로 `extract_report_lines()`를 호출해 fy별 성공/실패를
> 집계했다(원 4,366건보다 넓은 후보군 — v2 없이는 "v2가 실제로 값을 만들었었는지"를
> 더 이상 걸러낼 수 없어서, 정확한 재현이 아니라 "이 시대 포맷을 얼마나 못 읽는지"
> fy별 실측이 목적).
>
> **결과: fy1999~2017 전 구간에서 성공 5건/12,960건(0.04%)** — 1999~2002, 2005~2017
> 전부 성공 0건, 2003(3/1,520)·2004(2/454)만 미미하게 성공. **"더 이른 시기 서브포맷만의
> 문제로 범위가 좁혀진다"는 2026-09-02 결론은 표본 2건의 우연(대한광통신 2006Q3가 이
> 12,960건 후보군에 애초에 없었던 것으로 보임)이었고, 실제로는 전 구간에 걸쳐 거의
> 전멸 수준으로 실패한다** — 원 스코핑("99.8% 본문 섹션 없음")이 오히려 정확했다.
> (참고: 후보 모수 12,960건은 원 4,366건 스코핑과 직접 비교 불가 — v2 없이 재구성한
> 근사치이며 진짜 갭보다 넓을 수 있음. `missing_file`=71건은 `download_tasks`엔
> completed로 남아있는데 실제 파일이 사라진 별도 소규모 이슈, 이번엔 미조사.)
>
> **원문 대조로 확인된 실패 서브클래스(코드 없이 5개 표본 직접 열람, 전수는 아님)** —
> 단일 원인이 아니라 최소 3가지가 섞여 있다:
>   1. **FS 섹션 자체가 문서에 없음** — 한올바이오파마 1999FY(`20000329000397`)는 목차상
>      "III. 재무에 관한 사항 → 1. 요약재무정보"에서 파일이 그냥 끝난다(대차대조표/
>      손익계산서 섹션 자체가 이 XML에 없음, 5개년 요약표만 존재). 파서 버그가 아니라
>      이 문서에 정말 본문이 없어 보임 — 별도 첨부·감사보고서로 나갔을 가능성(이
>      파이프라인은 `annual`/`half`/`quarter` 세 report_type만 추적, 감사보고서를
>      별도로 안 받음 — 미확인, 조사 필요).
>   2. **진짜 빈 스텁 필링** — 아세아제지 1999H1(`20000120000003`)는 파일이 6.3KB뿐,
>      재무 내용 자체가 없음(정정·표지성 필링 추정). 실패가 아니라 정상.
>   3. **"통짜 텍스트 셀" 레이아웃(진짜 파서 갭)** — 한섬 1999H1(`20000117000001`)은
>      대차대조표/손익계산서 섹션이 실제로 존재하고 "가.대차대조표" 하위표제 인식(§상단
>      `legacy_pre2015.py` 근본원인 수정)도 이미 통과하는데, 정작 과목(라벨) 열 전체가
>      "자 산 Ⅰ.유 동 자 산 (1)당 좌 자 산 1.현금 및 현금등가물 …" 식으로 **한 칸(`<TD>`)에
>      고정폭 텍스트로 통째로 눌려 있다** — 행마다 별도 `<TD>`가 아니라 사실상 사람이
>      읽으라고 인쇄한 텍스트 블록. 현재 파서(2015+ 경로·pre-2015 K-GAAP 경로 둘 다) 는
>      "한 TD = 한 라벨"을 전제해 이 레이아웃을 아예 표로 인식하지 못한다 — `legacy_
>      pre2015.py`가 고친 것은 섹션 경계 리셋 버그뿐, 표 **내용** 형태는 다룬 적 없음.
>      이게 새 R-트랙이 붙어야 할 진짜 표적으로 보이나, 3건 중 1건 표본이라 비중 미확인.
>
> **다음 필요 작업(미착수, 사용자 결정 대기)**: (a) 서브클래스별 비중을 정확히 알려면
> 실패 12,955건 중 표본을 층화추출해 "FS 섹션 없음" vs "빈 스텁" vs "통짜 텍스트 셀"로
> 분류하는 재조사가 먼저 필요(지금은 5건만 열람), (b) 서브클래스 3(통짜 텍스트 셀)만
> 신규 파서 투자 가치가 있고 나머지 둘은 조치 불가/불요일 가능성, (c) 서브클래스 1(FS
> 섹션 자체 없음)이 실제로 별도 감사보고서 첨부 문제라면 이건 파서가 아니라 **수집
> 파이프라인**(새 document type 다운로드) 범위의 훨씬 큰 작업.

> **2026-09-03 후속 — 전수 서브클래스 분류 + 파일 절단(truncation) 재검사 완료, 근본원인
> 대폭 정정(코드 변경 없음, 조사만).** STRUCTURAL_EMPTY 12,867건 전수를 텍스트 휴리스틱으로
> 분류: **①NO_FS_TITLE 11,716건(91.1%)·②STUB 850건(6.6%)·③PACKED_CELL 207건(1.6%)·
> ④OTHER_STRUCTURED 94건(0.7%)**. 사용자가 ①/② 표본(한올바이오파마 20000329000397,
> 아세아제지 20000120000003)을 DART 웹 뷰어에서 직접 대조 — **실제 원문엔 재무제표
> 섹션이 정상적으로 있음**(파서가 예상한 위치 문제가 아니었음). 원문 XML 재검사 결과
> **두 파일 다 `<DOCUMENT>`/`SECTION-1`/`SECTION-2`가 안 닫힌 채 잘려있음**(TABLE/TR/TD는
> 짝이 맞음 — 표 하나 끝난 직후 잘림) — ①/② 버킷은 "원문에 없음"이 아니라 **로컬
> 저장 파일이 절단됨**이 근본원인이었다.
>
> `download_tasks` 대조: 12,960건 **전부** `last_error`가 동일하게 "파일 소실 —
> 재다운로드 대상으로 리셋"(개별 실패가 아니라 대규모 파일소실 사건 1건 이후 일괄
> 재다운로드된 것 — NAS 드리프트 계열 사건과 같은 시기로 추정, 미확인). DB `file_size`는
> 실제 디스크 파일과 12,960건 전부 정확히 일치(=재다운로드 시점에 이미 이 크기로
> 저장됐다는 뜻, "그 이후 파일이 또 훼손"이 아님) — `downloader.py::_download_one()`이
> ZIP 압축해제(CRC 통과, 우리 클라이언트 문제 아님) 후 **완결성 검증 없이 바로
> `completed` 마크**하는 게 재발 지점.
>
> 절단(태그짝 불일치) 전수 재검사 — **fy1999~2003에만 집중, fy2004~2017은 0%**:
> | fy | 절단률 | fy | 절단률 |
> |---|---|---|---|
> | 1999 | 77.6% | 2003 | 59.3% |
> | 2000 | 91.0% | 2004~2017 | **0.0% (전부)** |
> | 2001 | 78.3% | | |
> | 2002 | 77.5% | | |
>
> 서브클래스별로는 ①의 48.7%, ②의 100%, ③의 18.8%, ④의 5.3%가 절단 — 즉 ②STUB
> 850건은 "진짜 빈 필링"이 아니라 **전부 절단된 파일**이었다(분류 재정정: ②라는
> 이름은 유지하되 의미는 "작아서 헤더를 못 찾음"이 아니라 "절단으로 인해 작아짐").
>
> **결론**: fy1999~2003 실패분(다수)은 **파서가 아니라 DART `document.xml` API가 이
> 시기 문서에 불완전한 응답을 준 것**(ZIP은 CRC 통과 — 서버 쪽 원인으로 추정, 미확정)이
> 근본원인. 복구 경로는 이미 코드에 있음 — `collector/legacy_downloader.py::
> LegacyDartScraper`가 DART 웹 뷰어(`dsaf001/main.do`→PDF 우선→`/report/viewer.do`
> HTML 폴백)로 원문을 재수집하는 기능을 갖추고 있으나, 지금은 "014 오류"·"ZIP에
> xml/pdf 없음" 두 경우에만 자동 연결돼 있고 "ZIP은 받아졌는데 XML이 잘림"은 걸리는
> 조건이 아니다. 필요 작업(미착수, 승인 대기): (a) `_download_one()`에 완결성 검증
> (`</DOCUMENT>` 닫힘 등) 추가해 재발 방지, (b) 기존 절단분(1999~2003, 약 6,586건
> 추정)에 `LegacyDartScraper` 복구 패스 실행. **fy2004~2017 실패분(~4,353건, 절단
> 0%)은 이 원인과 무관한 별개 미해결 문제로 남음** — 파일은 완전한데 왜 여전히
> "본문 섹션 없음"인지 별도 조사 필요.
>
> 산출물: 전수 분류 CSV(`category_c_truncation_results.csv`, rcept_no별 서브클래스·
> 절단여부·파일크기·경로) — 세션 스크래치패드에 저장, 후속 세션 재사용 시 재생성
> 필요(스크래치패드는 세션별 격리).

> **2026-09-03 후속2 — PDF 복구 파이프라인 구현 완료, 전량 실행은 사용자 터미널에서
> 진행 중/대기.** 재시도 실측: 같은 `document.xml` API 재호출은 바이트 단위로 완전히
> 동일한 잘린 응답(314,140B/6,951B, DB 기록과 정확히 일치) — 결정적 실패, 단순 재시도
> 무의미. `collector/legacy_downloader.py::LegacyDartScraper`(DART 웹뷰어, PDF 우선)로
> 재요청하면 완전한 PDF 확보됨(표본 2건 확인). PDF 텍스트추출은 기존
> `fin2/extract/pdf.py::extract_pdf_facts()`(Track C, 원래 fact_v2용) 재사용, 신규
> `collector/pdf_lines_sync.py`가 그 산출물(`ExtractedFact`, canonical 매핑 완료)을
> `ReportLineRow`(raw-label 계약)로 역변환해 기존 v3 파이프라인(`store_report_lines`→
> `build_corp`)에 그대로 태움 — fact_v2 되살리지 않음.
>
> 구현 중 `fin2/extract/pdf.py` 기존 버그 2건 발견·수정(20000329000397 원문대조):
> ① `parse_number()`가 이 시대 BS 로마숫자/괄호번호 상위항목의 "소계 미리보기" 괄호
> ("Ⅰ. 유동자산 (45,700,051)")를 음수로 오판 — `_SUBTOTAL_HEADER_RE` 신설해 헤더 라인
> 단일 괄호값만 음수판정 제외. ② `_find_anchors()`가 붙여쓰기+자간띄움 두 표기("대차
> 대조표\n대 차 대 조 표")를 같은 앵커로 중복 매칭 — 근접 앵커 dedup 추가. 두 수정 다
> 표본 재검증 완료(BS 헤더 부호 전부 정상화).
>
> **잔존 노이즈(미수정, 위험 아님으로 검증됨)**: BS 리전이 당기표 다음 제N-1/N-2기
> 비교표까지 구조적으로 포함해, 그 비교표에서 라벨이 뭉개진 스퓨리어스 행이 소수
> 섞여 들어온다(예: `자본조정 Δ`). 앵커/리전 로직으로 완전 차단하려면 다른 필링(반기/
> 분기 2단헤더)을 오탐지할 위험이 있어 보류 — 대신 `build_corp()`의 기존 `_resolve()`
> 충돌감지 안전장치가 실제로 이를 막는지 **실측 검증 완료**: 표본 1(한올바이오파마)에서
> `total_assets`/`total_liabilities`/`total_equity`/`net_income`/`gross_profit`/`sga`
> 전부 원문과 정확히 일치, `is.cogs`만 후보 3개 충돌 감지돼 `conflicts` JSON에 플래그되고
> NULL로 안전하게 보류(틀린 값으로 확정되지 않음). 표본 2(아세아제지, 라벨 뭉개짐 더 심함)도
> `revenue`-`cogs`=`gross_profit` 항등식 정확 성립, 라벨을 못 읽은 개념(`total_assets`/
> `net_income`)은 틀린 값 대신 NULL로 결측(mapper 게이트가 매핑 실패 라벨을 애초에
> 버림) — **손상 아니라 결측이 최악의 실패모드임을 확인**, 대량 롤아웃 안전 판단.
>
> 소규모 스모크테스트(corp 6개 seed 상위 6건) 실행·커밋 완료: 6/6 PDF 복구 성공,
> report_lines 416행 적재, 에러 0. 처리속도 필링당 ~7.7초(DART 웹 쓰로틀링) → 전체
> 6,592건 잔여분 **예상 소요 13~14시간** — 장시간이라 `scripts/run_pdf_recovery_2026-09-03.py`
> 를 사용자 터미널에서 직접(또는 nohup 백그라운드로) 실행하도록 안내, 이 세션에서
> 직접 실행하지 않음(정책: 장시간 명령 미실행). 멱등(rcept 단위 delete-then-insert +
> "report_lines 없음"만 재조회) — 중단 후 재실행 안전.
>
> **완료 후 남는 작업**: report_tables 메타는 PDF 경로에서 의도적으로 스킵(table_seq
> NOT NULL 제약과 충돌, build_corp에 불필요 확인됨) — F3 표 메타가 이 rcept들에서는
> 영구 결측이나 낮은 우선순위. 전량 완료 후 dq_assertions(특히 `std_v3_conflicts_
> unresolved` 증가폭 — `is.cogs` 계열 충돌이 얼마나 흔한지)와 Gate B 격리성 확인 필요.

> **2026-09-05 — 전량 실행 완료 + 검증 완료.** 사용자가 이틀에 걸쳐 터미널에서 직접
> 실행(1차 2026-09-03 밤: 2,978건 처리 후 정수오버플로 미처리 예외로 중단 — `_download_
> one` 밖 `store_report_lines` 예외가 try/except 범위 밖이라 세션 전체가 죽음, 커밋
> `275274a`로 수정: ①`fin2/extract/pdf.py`에 값 상한 가드(|amount_won|>10^16 드롭)
> ②`pdf_lines_sync.py` try/except 범위를 store_report_lines까지 확장+실패시 rollback.
> 2차 2026-09-04~05: 나머지 5,490건 처리, **에러 0건**으로 완주).
>
> **최종 결과**: 후보 8,514건(922개사, fy1999~2003) 중 **5,830건(68%) 복구 성공**
> (report_lines 474,912행, `unit_source='pdf'`). **2,608건(31%)은 DART 웹뷰어에 PDF
> 자체가 없어 HTML만 확보**(원문 보존만, 파싱은 애초 스코프 밖 — 사용자 결정
> "PDF 경로만"). 4건은 원문 자체를 못 구함(no_content).
>
> **dq_assertions 전수 검증**: `statement_magnitude_impossible`(ERROR) 32건 중
> 오늘 신규 6건만(나머지 26건은 무관한 기존 이슈, 항목2) — 같은 부류의 PDF 자릿수
> 뭉개짐이 값 상한가드(10^16)보다 작게(9,000조원대) 새어나온 것. 재발방지 코드
> 커밋(`8c3d8fb`, dq_assertions와 동일 임계값을 concept별로 재사용해 추출 시점에
> 차단)했으나, **기존 6건의 DB 정리(재수집)는 DART PDF 엔드포인트가 그 시점에 503을
> 내서 미완료 — 나중에 재시도 필요**(무리한 재시도 자제, `docs 항목 검색용 corp_code`:
> 00122825/00124799/00125488/00133618). `bs_identity_gt5pct`(WARN) 947건 중 277건,
> `std_v3_conflicts_unresolved`(WARN) 오늘범위 10,532행 중 4,054행(38.5%)이 오늘
> 신규분 — 전부 "이전엔 아예 없었던 자리가 불완전하게라도 채워짐" 패턴이지 기존 데이터
> 손상이 아님(격리성 확인: 새 report_lines는 전부 이전에 완전히 비어있던 (corp,fy,fp)
> 자리만 채움, `unit_source='pdf'`를 특별취급하는 코드는 fx_declared 가드 외 없음 확인).
>
> **이 항목 사실상 종료** — 남은 서브이슈 2개만 낮은 우선순위로 백로그: (a) 6건
> 재수집(DART 503 풀리면), (b) 2,608건 HTML-only 파싱(신규 파서 필요, 별도 결정
> 필요 사안).

> **2026-09-05 — 항목1-b(fy2004~2017, 절단률 0%) 원인규명 완료(코드 변경 없음).**
> 재조사 결과 후보 규모부터 갱신: `download_tasks`(xml/zip completed) × `filings` 조인
> 후 `report_lines` 미존재 rcept 를 fy2004~2017 전 구간에서 재추출한 결과 **5,739건**
> (원 스코핑 ~4,353건과 다른 근사치 — v2 DROP 이후 재구성이라 정확 비교 불가, §3 위
> 후속 기록과 같은 사정). fy별 분포: 2004=474·2005=281·2006=190·2007=99·2008=109·
> **2009=3,970(69%, 압도적 최다)**·2010=296·2011=52·2012=85·2013=101·2014=80·
> 2015=1·2017=1.
>
> 5개 표본(2005/2007/2009×15/2011/2014, corp·rcept 다양)을 실제로
> `extract_report_lines()`/`_detect_body_statement_tables()`/
> `_detect_legacy_body_statement_tables()`/`assign_tables_to_dart_sections()`에
> 직접 통과시켜 추적한 결과 **단일 원인이 아니라 서로 다른 시대별 XML 레이아웃
> 변종 5종**이 확인됐다(전부 "파서가 알던 구조가 이 변종에서 살짝 다르게 생겨서
> 못 알아본다" 유형 — §3 앞부분의 truncation/빈스텁과는 완전히 다른 실패모드):
>
>   **(A) SECTION-3 서브헤딩이 상위 섹션 분류를 지운다** — 2005~2007 연간/반기
>   서식(예 호텔신라 `20050915000066`, SK네트웍스 `20080331001324`)은 `4.재무제표`/
>   `5.연결재무제표`(SECTION-2, `assign_tables_to_dart_sections`가 정확히
>   `재무제표`/`연결재무제표`로 분류함)**안에 BS/IS/CF 각각을 별도 SECTION-3
>   표제("가.대차대조표","나.손익계산서"...)로 중첩**해 넣는다.
>   `assign_tables_to_dart_sections`는 TITLE 있는 SECTION 을 만나면 무조건 분류를
>   갱신(미분류 표제=`current` 해제)하는 규칙이라, 이 서브헤딩(분류사전에 없음)을
>   만나는 순간 `current`가 None 으로 리셋돼 그 뒤 BS/IS/CF 데이터표가 전부
>   미귀속으로 떨어진다. 실측: `assign_tables_to_dart_sections` 결과에 `재무제표`/
>   `연결재무제표` 키가 아예 없음(표는 `요약재무정보`에만 2개 귀속).
>
>   **(B) 한글 가나다 열거 접두 미제거** — `_LEGACY_ENUM_PREFIX`(`fin2/extract/
>   statement_titles.py:413`)는 숫자·로마숫자 접두("1.","29.","Ⅲ.")만 걸러내고
>   "가.","나.","다.","라.","마." 접두는 그대로 통과시키는데, 그 다음 단계인
>   `_LEGACY_HEAD`(재무제표명 매칭)는 문자열 **시작**에서 매칭해야 해서 "가."가
>   앞에 붙은 "가.대차대조표"를 통째로 놓친다(2011 카페24 `20111115000196`,
>   `XI.재무제표 등` 섹션 안 "가.대차대조표"/"나.손익계산서"/"다.이익잉여금처분
>   계산서"/"라.현금흐름표"/"마.자본변동표" 5개 헤딩 전부 미인식 → 데이터표 4개
>   전부 미귀속, `classify_legacy_statement_heading` 직접호출로 재현).
>
>   **(C) "요약" 접두가 `_LEGACY_HEAD` 수식어 목록에 없음** — 2014 삼성증권
>   `20140515001582`(분기보고서)의 헤딩이 "요약분기연결재무상태표"/"요약분기연결
>   포괄손익계산서" 등인데, 허용 수식어(연결·별도·개별·반기·분기·중간·당·전)에
>   "요약"이 없어 문자열 시작 매칭이 실패 — 증권/보험 등 대형사 분기보고서에 흔한
>   "요약재무제표" 표기 계열 전체가 이 갭에 걸릴 가능성.
>
>   **(D) 괄호 병기 표제 미인식** — 에너토크 2009반기 `20090814000347`의 BS 헤딩이
>   "재무상태표(대차대조표)"(IFRS 전환기 신명칭+구명칭 병기)인데, `_LEGACY_HEAD`는
>   "재무상태표"까지는 매칭하지만 그 직후 `_LEGACY_PERIOD_AFTER`(기간/단위 마커
>   기대)가 "(대차대조표)"를 못 받아들여 헤딩 전체가 기각됨.
>
>   **(E, 규모 최대) 레거시 컨테이너명 변종 "XI.부속명세서"** — `iter_section_
>   elements(root, SEC_LEGACY_FS)`는 SECTION-1 표제가 정확히 "재무제표등"
>   (="XI.재무제표 등")인 경우만 인식하는데, fy2009 분기/반기보고서 다수(무작위
>   15/15 전수 확인, `20090515000636` 외)는 재무제표를 "XI.재무제표 등"이 아니라
>   **"XI.부속명세서"** SECTION-1 아래 TITLE 로만(SECTION 재중첩 없이) 나열한다
>   ("재무상태표(대차대조표)"→[제목표][데이터표], "손익계산서"→…,
>   "이익잉여금처분계산서/결손금처리계산서"→…, "자본변동표"→…, "현금흐름표"→…
>   — 헤딩·데이터표 인접구조 자체는 `XI.재무제표 등`과 동일, 컨테이너 표제 문자열만
>   다름). `SEC_LEGACY_FS`가 "재무제표등" 하나만 인정해 이 컨테이너 자체를 못 찾고
>   `_detect_legacy_body_statement_tables`가 빈 dict 반환 — fy2009 3,970건(전체
>   후보의 69%)의 지배적 원인으로 추정(15/15 표본 전수 일치, 나머지 fy 대비
>   압도적 비중과도 부합). (D)와 겹치는 문서도 있어(에너토크 예시가 바로 E+D 동시
>   해당) 완전히 배타적인 카테고리는 아님.
>
> **다음 필요 작업(미착수, 사용자 결정 대기)**: (a) 전수(5,739건) 대상 정량
> 버킷분류 스크립트를 백그라운드 실행 중(스크래치패드 `classify_category_c.py`/
> `.log`, NAS 마운트라 매우 느림 — 1,000/5,739에 ~24분, 완주 시 정확한 5종 비중
> 확정 가능, 세션 스크래치패드라 후속 세션엔 재실행 필요), (b) 코드 수정은 5종
> 각각 좁고 안전한 가드로 가능해 보이나(예: (B)는 `_LEGACY_ENUM_PREFIX`에 가나다
> 접두 추가, (E)는 `SEC_LEGACY_FS` 인정 표제에 "부속명세서" 추가) **구현은 미착수,
> 승인 대기**(CLAUDE.md 정책 — 계획 후 대기). 특히 (E)가 fy2009 대부분을 좌우하므로
> 우선순위 후보.

## 4. `extended_financials_n_facts_outlier` 어서션 폐기, 대체 설계 미착수

§4-2 `extended_financials` 뷰를 fact_v2(acode) 경유 → `extended_facts_v3`(라벨) 경유로
재정의하면서 이 어서션이 상시무의미화돼 함께 폐기됐다. 대체 설계는 별도 소규모 백로그로만
분리해두고 아직 손대지 않음.

**영향도**: 낮음~중간(정확히 무엇을 감지하던 어서션이었는지, 대체가 필요한지부터 재확인
필요). **난이도**: 낮음(어서션 1개 재설계).

> **2026-09-02 진행 — 완료.** 설계문서 §4-①이 제안한 대로 `std_financials_v3.conflicts`
> (canonical별 combine.py `_resolve()`가 값 충돌로 보류한 후보 목록, 코어+확장 캐노니컬
> 공통) 기반 새 WARN `std_v3_conflicts_unresolved` 신설. 실측 35,305행(전체의 11.6%)에
> 실제 충돌 존재, 최빈 canonical=bs.intangibles(12,687)/is.cogs(10,188)/is.sga(8,460)/
> is.noncontrolling_ni(3,132). 옛 어서션과 신호 성격이 다름(무관 라인 다중합산 → 값 충돌로
> 보류된 canonical 존재)을 코드 주석에 명시. `scripts/dq_assertions.py` 워킹트리 변경
> (커밋 안 함).

## 5. `statement_magnitude_impossible` 150건 위반 — std_v3 자체 기존 데이터 품질 이슈, DROP 이후 새로 가시화

std_v2 DROP 검증 중 발견. 표본(`00366942` FY2004H1 이익잉여금 9,950조원류) 확인 결과
이번 DROP이 만든 문제가 아니라 std_v3 자체의 기존 단위오염 데이터 — 이 어서션이 지금까지
v2만 보느라 몰랐던 v3측 신규 가시성일 가능성. 별도 백로그로 남겨져 있고 미착수.

**영향도**: 중간(단위오염이 실제 화면 노출 지표에 영향 줄 수 있음). **난이도**: 미확인
(원인 규명부터 필요).

> **2026-09-02 진행 — 원인규명 완료(코드 변경 없음).** 150건 전수 실측: **72개사, 전부
> fy 2000~2007 분기/반기(Q1/Q3/H1) 별도재무제표** — assets/equity는 NULL·0인데
> retained_earnings 또는 revenue 한 필드만 조(兆) 단위로 튀는 패턴이 지배적(전형적
> "일부 필드만 오염, 나머지는 결측"). 대한광통신 2006Q3(`20061114000051`) 표본을
> 원문(EUC-KR, `iconv`로 디코딩 — `sanitize_dart_xml`/[[feedback-grep-euckr-locale-trap]]
> 함정과 동일 계열)까지 대조: **root cause = 옛(`<DOCUMENT>`) 포맷 표에 붙은 단위선언이
> plain-text `(단위 : 백만원)`(XML 속성 `AUNIT` 없이 텍스트로만 존재)인 경우, 현재
> 파서가 그 표 자체의 선언을 못 읽고 문서 레벨 기본단위(doc_default)로 폴백해 ×10⁶을
> 잘못 적용**(추출 결과 실측: `unit_source='doc_default'`, `adecimal=-6`, 이익잉여금
> 5,229,469,085,000,000원 — 원문 표는 정상 파싱됐고 문제는 그 표에 붙은 단위 텍스트를
> 못 읽는 것뿐이었음, "본문 섹션 없음"으로 실패한 §3과는 다른 실패모드). 이미 알려진
> 단위오추측 버그류(a)(`unit_contamination` 어서션 주석 참고)의 **옛 포맷 전용
> 변종**으로 보임 — 정식 수정은 `_PRE2015_ROUTING_MAX_FY` 경로의 단위 추출 로직(header_
> hint) 확장이 필요해 신규 파서 작업급(별도 R-트랙), 이번엔 원인규명까지만.

## 6. `calendar_orphan_cq` 어서션 SKIP 유지 — 이산분기/달력 정책 재설계 필요

`std_financials_v2`의 `is_discrete` 개념을 묻는 어서션인데 그 쓰기 경로 자체가 Phase 2부터
멈춰 있어 v3에 포팅 대상이 없다. 이산분기·달력(`std_financials_calendar`) 자체는
**실사용 중**(`screen_window.py`/`series.py`/`quarter_change.py`/`company_page.py`, §5-d
정정 확인)이라 폐기는 아니고, 유지하려면 이 4개 소비자의 v2→v3 이식이 선행돼야 한다.

**영향도**: 낮음(현재 크래시나 데이터 손실 없음, 단 회귀 감지 공백은 있음). **난이도**:
중간(소비자 4곳 이식 + 어서션 재설계, 별도 트랙).

> **2026-09-02 진행 — 조사 결과 원 스코핑이 부정확했음을 발견, 구현 보류(사용자 결정
> 필요).** 실제로 확인해보니 4개 소비자(`screen_window.py`/`series.py`/`quarter_change.py`/
> `company_page.py`)는 `calendar_financials` 뷰(→`std_financials_calendar`, DROP 안 된
> 별개 테이블)만 읽고 `std_financials_v2`를 안 읽는다 — **이식할 소비자 자체가 없다.**
> 진짜 쓰기 경로는 `fin2/standardize/calendar.py::calendarize_corp()`(`_load_discrete()`가
> `std_financials_v2`를 SELECT)인데, 이 함수는 **§4-1(계층2 GC 조사, 커밋 `97ff39d`)에서
> 이미 RuntimeError 가드가 걸렸고, 그보다 먼저 Phase 2(2026-08-30)에서 데일리 파이프라인
> 배선 자체가 끊겨 있었다** — 즉 `std_financials_v2` DROP 훨씬 전부터 `std_financials_
> calendar`는 신규 기업/신규 분기로 자라지 않는 정지 테이블이었다(실측: `calendar_year`
> 최댓값 2026, 319,694행 — Phase 2 중단 직전까지의 데이터는 있어 당장 화면이 텅 비진
> 않지만, 그 이후 신규 공시분은 계속 안 쌓이고 있다). **이건 v2 DROP이 만든 문제가
> 아니라 그보다 먼저 있었던, 지금도 계속 진행 중인 별개의 조용한 결측이다.**
>
> 남은 선택지 2가지, 둘 다 GC 백로그보다 큰 결정이라 사용자 판단 필요:
> ① **은퇴** — 이산분기/달력 기능 자체를 접고 4개 소비 화면도 함께 정리. 근거: 원래도
>    "이산분기·달력 사용자 노출 없음"이 1차 결론이었다가 뷰 참조가 있어 정정된 것뿐,
>    실질 활용도는 재확인 필요.
> ② **v3 재설계** — `calendarize_corp()`(및 자매함수 `derive_quarters_corp()`)를
>    `std_financials_v2` 대신 `std_financials_v3`를 읽도록 다시 짜고 데일리 배선 복구.
>    새 파이프라인 작업급(런북 3층: 배선+소급백필+검증) — 이번 세션 범위를 벗어남.
> `calendar_orphan_cq` 어서션 자체는 어느 쪽을 택하든 그 이후에 따라오는 부수 작업이라
> 이번엔 SKIP 유지, 코드 변경 없음.

## 7. (참고, 보강 불요) `cf_da_sync.py` D&A 계열 은퇴 — 기능적 손실 없음 확인됨

`note.depreciation`/`amortization`/`rou_depreciation`/`da_total`는 은퇴(D) 결정됐으나,
확인 결과 이미 `std_financials_v2` DROP 시점부터 죽은 쓰기였고 v3 D&A(`note_da.py`)는
애초에 `fact_v2`를 안 읽고 `note_lines`만 읽어 만든다 — 별도 조치 불필요. (리스트에는
완결성을 위해 남겨두되, 착수 후보에서는 제외.)

---

## 2. 권장 우선순위 (제안, 착수는 사용자 선택)

| 순위 | 항목 | 이유 |
|---|---|---|
| 1 | §1 dq_assertions 2건 재구현 | 회귀탐지 공백 — 과거 실제 재발 이력 있는 버그 클래스, 표면적 좁음 |
| 2 | §5 statement_magnitude_impossible 150건 원인규명 | 실제 지표 오염 가능성, 원인규명만 먼저(저비용) |
| 3 | §3 Category C 겹침 확인 | 새 작업 벌이기 전 기존 트랙 재확인이 선행조건(저비용, 조사만) |
| 4 | §4 extended_financials_n_facts_outlier 대체설계 | 규모 작음 |
| 5 | §6 이산분기 소비자 4곳 이식 | 현재 기능 정상 작동 중이라 급하지 않음 |
| 6 | §2 v2 UNION 12,149건 정책 재검토 | 되돌릴 수 없는 트레이드오프, 가장 큰 설계 결정 필요 — 신중 |

## 참고

- 관련 메모리: `factv2-stdv2-gc-scoping-2026-09-01`, `gateb-factv2-sync-scripts-track1-2026-09-01`,
  `gateb-phaseb-line-audit-migration-phase0-1-2026-09-01`.
- 원 스코핑 문서: `docs/plans/factv2_stdv2_gc_scoping_2026-09-01.md`.
