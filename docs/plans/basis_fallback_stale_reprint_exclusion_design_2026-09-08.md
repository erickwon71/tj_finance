...
2. ~~버킷A 35건/27개사~~ **✅완전 종료(2026-09-08, 같은 날 후속 세션)** — 상세는
   아래 "버킷A 35건/27개사 삭제+재빌드 완료" 절 참고.
3. **오늘 고친 코드의 전사 반영**(재빌드) — 아직 안 함. 스코프 결정 필요(68개사만 /
   fy≤2012 전체 / 전사).

## 버킷A 35건/27개사 삭제+재빌드 완료 (2026-09-08, 같은 날 후속 세션)

사용자 지시로 진행("버킷A 35개사 report_lines 삭제하고 재빌드해줘"). DKME와 동일
메커니즘(그 시대 연결 인터림 작성의무 없음 — report_lines의 "연결" 행은 실제
당기 데이터가 아니라 원문상 오귀속된 값)으로 원문대조 없이 바로 진행.

**census 재현**: `std_financials_v3`에서 fy≤2010, 연결 인터림(Q1/H1/Q3) revenue가
직전FY(FY) revenue와 원 단위까지 정확 일치 + `report_lines`에 그 rcept·consolidated
행이 존재(=버킷A 판정, 없으면 버킷B). DB 직접조회로 재현 결과 **정확히 35건/27개사**
일치 확인(문서 수치와 100%). 목록: 삼성증권·뉴인텍·LS·농심·대구백화점·대창단조·
KG스틸·동원모빌리티·티에이치엔·롯데쇼핑·삼성생명·녹십자·SBS·SK증권·송원산업·
신세계·신영증권·차AI헬스케어·유니크·제주은행·진흥기업·코리아나·피에스텍·SK·
팜스토리·카카오·CJ CGV.

**1차 시도(불충분)**: `std_financials_v3.source_rcepts.IS`가 가리키는 rcept
1개만 콕 집어 `(corp, rcept, basis='consolidated', fiscal_year, fiscal_period)`
스코프로 삭제 → 재빌드. 35건 중 25건은 이걸로 정상 해소(NULL 또는 다른 값)됐지만,
**10건은 그대로 오염값 유지**됨을 직접 대조(before/after 값 비교, census 재실행
지문 매칭이 아니라)로 발견 — 지문 재매칭 방식은 전기(FY-1) 값 자체도 같이 바뀌는
바람에 "해소된 것처럼" 잘못 보이는 함정이 있어(재현 스크립트 버그로 처음엔 이걸
못 잡을 뻔함) 원래값 vs 현재값 직접대조로 재검증.

**원인**: 10건 중 8건은 같은 (corp,fy,period)에 **다른 rcept가 형제로 더 있고
거기도 동일한 오염값**을 갖고 있어서, 하나만 지워도 `combine_full()`이 남은
형제 rcept에서 같은 오염값을 다시 확정. 나머지 2건(00104856 삼성증권·00136721
신영증권, 둘 다 증권업)은 표면적으로는 형제 rcept의 최상위 "영업수익" 라벨값이
아예 다른 숫자였는데도 최종 revenue가 옛 오염값과 똑같이 나와 처음엔 미스터리였음
— 추적 결과 **증권업 industry_profile revenue 공식(=operating_income+sga
컴포넌트 합산)이 형제 rcept의 (똑같이 오염된) operating_income/sga 성분으로
같은 총액을 재구성**하고 있었음(00104856: 132,053,823,489+480,403,535,971=
612,457,359,460, 정확히 옛값과 일치, 직접 검산 확인). 즉 오염이 특정 rcept
문제가 아니라 **그 기간 자체**의 문제 — 근본원인 서술("그 시대 연결 당기데이터
자체 없음")과 정합.

**2차 삭제(전체 기간 스코프)**: 남은 10건은 `(corp, basis='consolidated',
fiscal_year, fiscal_period)`로 스코프를 넓혀(rcept 구분 없이) 그 기간의 연결
데이터를 통째로 삭제 → 재빌드. **35/35건 전부 원래값과 다르게 바뀜(대부분 NULL
또는 훨씬 작은 정상 규모 값으로) 최종 확인**.

**검증**:
- report_lines 삭제 총 8,576행(1차 6,282 + 2차 2,294), corp 27개사 전체
  `build_std_v3.py`급 재빌드(`build_corp(year_min=1999)`) + `calendarize_corp_v3`
  완료.
- manual 데이터 보호가드 저촉 없음(사전 확인: 대상 8,576행 전부 `unit_source`
  ∈ {declared, pdf, section_def}, `manual` 0건).
- `dq_assertions.py` 전수: ERROR 1건(`valuation_daily_stale`, 주가 동기화 지연 —
  이번 작업과 무관한 별개 이슈), `statement_magnitude_impossible` 여전히 0건
  (회귀 없음).
- 코드 변경 없음(순수 DB 작업) — 커밋 대상은 이 문서 갱신뿐.

**교훈(다음에 이런 "report_lines 삭제+재빌드" 패턴 쓸 때)**: (1) `std_v3.
source_rcepts`가 가리키는 rcept 1개만 지우면 부족할 수 있다 — 같은
(corp,fy,period)에 형제 rcept가 있는지 항상 먼저 확인하고, 있으면 기간 전체
스코프로 지울 것. (2) 검증은 "지문 재매칭"이 아니라 **원래값 vs 현재값 직접
대조**로 해야 한다 — 지문에 쓰인 비교대상(여기선 전기FY값)이 같이 흔들리면
지문 재매칭은 거짓 음성을 낸다.
