...
| 항목 | 상태 |
|---|---|
| **캠페인 큐 — `status='pass'` 인데 `verified_scopes` 결측 487건** | **분해 완료, 24건만 재대조 진행(사용자 결정 2026-09-22)**. 전수 조사: pass 총 1,239건 중 487건 결측. ★**463건(2026-09-09 ~ 09-20 12:45)은 부실 근거가 아니다** — `verified_scopes` 컬럼이 마이그레이션 `2026_09_20_l2rq_verified_scopes` 로 09-20 13시경 **신설**됐고 그 이전 pass 는 NULL 이 당연하다(손대지 않는다). 남은 **24건(09-20 13:05~15:31)** 은 컬럼이 있는데도 비었고, 흩어진 게 아니라 그 오후 두산에너빌리티 20건·신한지주 4건 한 덩어리다(플래그 의무화 전환 구간). 이 24건만 캠페인 세션이 8-scope 재대조 + `--verified-scopes` 채우기. ★이들은 `pass` 라서 `redo` 가 값을 덮지 못한다(R139 가드) — 재적재가 필요하면 사용자 승인 후 `scripts/apply_r162_r163_reviewed_5.py` 와 같은 방식으로 처리한다. 함께: SK하이닉스 `20191114002661`(09-12 pass, 컬럼 이전 + R162 로 값 변경) 재대조, 두산에너빌리티 `20170811000844`(status='blocked' + NULL = **한 번도 pass 된 적 없는 미완료 리뷰**, 값은 SCE 음수 60/60 정상) 원문대조 후 pass |
