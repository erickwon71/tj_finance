...
> **상태: 실행됨 (2026-07-31, 커밋 `14cd8e7`).** 아래 설계 그대로 반영했고, 두 가지만
> 좁혔다: ① `keep_header_rows=True` 는 **주석 경로만** 켰다(본문·SCE 는 헤더 규칙이 실제로
> 헤더를 잡고 있어 위험 대비 이득이 없다) ② `note_periods` 의 '기간라벨 적극 활용'(§4)은
> 아직 안 했다 — 계층3 가드가 기본 제외라 **현재 동작은 종전과 같다**(회귀 0).
> 실측 회복량 = note 행의 **0.88%**(표본 40 filing · 982행). 소급 반영은 Phase 4 재적재.
>
